...
에디터의 임시저장은 localStorage에 있어 **파일로 내보내기 전까지는 브라우저 안에만 존재한다.**
사용자에게 안내할 것: `write.html`을 열어 `.editor-status`가 "내보내지 않은 변경 있음"인지 확인하고, 그렇다면 먼저 내보내라고. 브라우저 데이터를 지우면 그 초안은 복구 불가다.
