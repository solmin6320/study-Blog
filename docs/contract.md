...
> **v4.5 (2026-09-26) — 제목 타자 박자 조정. 숫자 넷만 바뀐다(클래스·DOM·문구·저장 키·키 동작 변화 없음).**
> 사용자 원문(v4.4 미리보기를 보고): *"메모블로그 글자가 너무 빨리 나와 속도 조금만 줄이고 커서 깜빡이는 시간 살짝만 늘려"*.
> ① 글자 간격 120 → **160ms** · 첫~끝 글자 상한 1200 → **1600ms**(같은 비율 4/3) ② 깜빡임 한 번 `--dur-caret-blink` 1000 → **1100ms**(켜짐 550 + 꺼짐 550), 횟수 **3 그대로** → 3초 → **3.3초**
> ③ 안전망 첫 글자 뒤 **5000ms 그대로**. 제목 6글자(`메모 블로그`) = 첫 글자 600 · 끝 글자 1400 · 평문 **4700**(v4.4 4200) / 상한 제목 = 첫 글자부터 평문까지 **4900**(< 5000).
> **5초 예산을 재는 기점을 "첫 글자"로 명시했다**(v4.4는 6글자 기준 첫 페인트부터 4.2초로 적었다) — 근거는 §3-5 "5초 예산". 실제 제목(6글자)은 첫 페인트부터 재도 4.7초라 5초 안이다.
> 바뀐 절만 읽으려면: §1-2 원칙 4 · §2 `--dur-caret-blink` 행 · **§3-5 ② 시간표 · 간격 · "5초 예산"(신설 문단)** · **§8-1 제목 타자 행** · §11 커서 행 · **§12-21(#188-189)** · §13.
> CSS는 `tokens.css` 값 하나 + 주석(`components.css` §16 주석). **JS는 frontend-dev가 `app.js` 상수 둘**(`TYPE_STEP_MS` 160 · `TYPE_SPAN_MS` 1600 — `TYPE_NET_MS` 5000은 그대로).
> **이행 상태(2026-09-26)**: #188(CSS · 계약) ✔ · #189(`app.js`) ☐. #189 전의 화면은 글자 120ms + 깜빡임 3.3초 — 예산 안(1200 + 3300 = 4500 < 5000)이라 깨지지 않는다.

> **v4.4 (2026-09-25) — 전구 인트로 폐기 · 첫 방문 연출 = 페이지 올라오기 + 제목 타자.**
> 사용자 원문: *"전등 소리 추가만 해 / 소리는 인터넷 서치나 아무걸로 전등 소리만 가져와"* → (첫 방문의 0.85초 자동 점등은 브라우저 자동재생 차단으로 소리가 나지 않는다는 안내) →
> *"그냥 전등 빼고 자연스럽게 렌더링 되는 다른 애니메이션으로 바꿀까?"* → 제안(전구를 빼고 올라오기만 0초부터)에 *"페이지 올라오기로 ㄱㄱ"* ·
> *"그 학습 블로그라고 메인화면에 되어 있잖아 그거 한 글자씩 나오면서 마지막에 커서 깜빡거리는 것도 추가하자"* · 재확인 *"전구 자체를 빼"*.
> **사용자가 확정했던 축 "인트로 전구"는 사용자 본인의 결정으로 폐기됐다**(2026-09-25). 막·전구·건너뛰기·`inert`·Esc와 그 토큰·CSS·키프레임, §1-2의 전구 예외 둘이 전부 사라진다.
> 남는 것은 둘이다 — ① 세션에서 index.html을 처음 여는 로드에 **골격 셋이 12px 아래에서 0.6초에 떠오른다**(0ms부터) ② 다 올라온 순간 **제목이 한 글자씩 쓰이고 커서가 세 번 깜빡인 뒤 사라진다**.
> 제목 문구(`메모 블로그`)는 바꾸지 않는다. **소리는 넣지 않는다.**
> 바뀐 절만 읽으려면: **§1-2(원칙 2·3의 전구 예외 삭제 · 원칙 4 모션 목록)** · **§2(인트로 토큰 여덟 · `--c-glow` · `--z-intro` 삭제 → `--dur-intro` · `--dur-caret-blink`)** ·
> §3(셸 마크업 첫 줄) · §3-1(스킵 링크 = 세 페이지 모두 첫 자식) · §3-2(저장 키 표 `intro` 행) · **§3-5(전면 개정 — 첫 방문 연출)** · §4-2 · §4-6 ⑧ ·
> §7(`.title-type` `.title-typed` `.title-rest` 등록 · 인트로 부품 폐기) · §8 · §8-1 · §8-2 · §11 · §11-1 · **§12-20(작업 #180-187)** · §13.
> **개발자 사이의 접점은 `html[data-intro]`의 뜻 하나다**(§3-5 "조건"). frontend-dev = index.html · `app.js` 타자 · `theme-init.js`/`config.js` 주석, frontend-dev-2 = `ui.js` `initIntro()` 축소.
> **이행 상태(2026-09-25)**: #180-183(계약 · CSS) ✔. #184-187 ☐. CSS가 먼저 바뀌었으므로 **#184 · #187이 끝나기 전의 index.html은 옛 `.intro` 마크업이 스타일 없이 남는다**
> (헤드리스 실측: 첫 방문에 `건너뛰기` 글자 버튼이 맨 위 왼쪽에 약 0.5초 보이고, 옛 `ui.js` 안전망이 치우는 순간 페이지가 그 줄만큼 올라간다 · 타자 없음). 두 작업이 끝나기 전에 배포하지 않는다.
> **이행 상태 갱신(2026-09-26)**: #184-187 구현·검증 완료 → **#180-187 전부 ✔**(§12-20). 위 "배포하지 않는다" 조건은 풀렸다. 박자 숫자는 v4.5가 바꿨다(맨 위 블록).

> **v4.3 (2026-09-23) — 라운드 8 통합 회의(`docs/meeting-08.md`) "다음 라운드 작업 지시 → web-designer" 2~7번. 사용자 원문: *"그냥 싹다 고쳐"*.**
> 축은 셋이다. ① **덮어쓰기 차단(D1)** — 새 글 id가 기존 글과 겹치면 서버가 소리 없이 덮어쓰던 경로를 에디터(사전 확인 · 자동 접미사 · `#fIdError`)와
> 서버 409 코드(`docs/api.md` v1.2 — `exists` · `stale` · `conflict`)의 **사용자 문구**로 막는다. ② **에디터 판정을 렌더러와 같게(D3·D17·D18·D19·D20·D21·D4)** —
> "코드 블록 안"을 marked가 `<pre><code>`로 그리는 범위(`Blog.markdown.codeRanges`)로, "줄 시작"을 공식이 아니라 **동작 문장**으로 정의한다.
> ③ **글쓴이의 실제 습관을 검증 입력으로(D16·R-C·R-H)** — §0-4 사용자 습관 목록 신설, `라벨:` + 목록 간격(`prose.css`), 깨진 frontmatter의 방문자 화면(D5).
> **사용자 결정(2026-09-23)**: U2 = (A) 에이전트 대필 경로 폐기(푸터 §10 문장 그대로) · U3 = (c) 요약 카드 현행(§5-7-2 — 문단 라벨형 B8-10 하지 않음) ·
> U4 = 이미지는 "규칙만"(§5-7 "이미지 경로") · U11 = 본문 칸 맞춤법 꺼짐 유지. 이월 백로그(meeting-08 B8-*)는 이 개정의 범위 밖이다.
> 바뀐 절만 읽으려면: **§0-4(신설 — 사용자 습관 목록)** · **§3-2(저장 키 표 신설 — `codeLang` 등재 · `admin` 삭제 · `store.draft.key`)** ·
> **§5-6(깨진 frontmatter의 화면 — 신설 소절)** · **§5-7(라벨 + 목록 · 느슨한 목록 · 이미지 경로)** · §5-7-2(트리거 행 정정 — U3) ·
> **§6-0(`#fIdError` · 오류 규칙 행 · 접힌 `.editor-more` 예외)** · **§6-1(저장 순서 · 409 코드별 문구 · 자동 접미사 · 덮어씀 경고 · 상태줄 문구)** ·
> **§6-2(머리 조합 문장 · Ctrl+S · Tab · `//` · 툴바 코드 블록 행 / 신설 "코드 블록 안 = `codeRanges`" · "줄 시작" · "편집 뒤 선택" · "조합 확정 뒤 건너뛰기" /
> 주석 표 확장 · 모르는 언어 = 무동작 + 토스트 / 언어 기억 대소문자 / Enter 공통 전제 / `~~~` 문장 폐기)** · **§12-19(작업 #160-179)** · §13.
> CSS 변화: `prose.css` §4 목록 넷(라벨 + 목록 12px · 느슨한 목록 항목 안 문단). 클래스·DOM 신설은 `#fIdError` 하나(기존 `.field-error` 부품).
> **이행 상태(2026-09-23)**: #160(CSS) · #161(계약) ✔. #162-179 ☐ — 서버 필드명은 `docs/api.md` v1.2(pm 작성 중)를 따른다. 이 문서의 이름과 다르면 api.md가 이긴다.

> **v4.2 (2026-09-23) — 코드 블록 편의(본문 `#fBody`의 단축 입력). 이미 구현된 동작의 사후 등재 — 새 기능을 설계하지 않았다.**
> 사용자 원문: *"코드 블록 부분 괄호 쓰면 IDE 처럼 괄호 닫히는거 까지 같이 보여줘"* · *"게시글 작성 부분을 좀 더 고도화(코드 블록을 특히나)"* ·
> 이번 세션 *"확정 되있는 부분만 작업 해줘"*.
> frontend-dev-2가 `editor.js`에 먼저 구현했고(머리 주석 v4.2), 계약서의 v4.1 §6-2 두 문장 — *"자동 닫기 괄호는 넣지 않는다"* · *"여기 없는 단축 입력을 만들지
> 않는다"* — 과 코드가 충돌했다. 이 개정은 코드를 계약에 올려 충돌을 없앤다. **코드 블록 편의 몫(#150-156)은 클래스·DOM 구조·CSS 변화 없음** —
> write.html에서는 `#bodyHint` 문장 하나만 바뀐다. (v4.2 전체의 마크업·CSS 변화는 아래 "추가 확정"의 푸터 둘째 줄 `<p>` 하나와 `components.css` 간격 두 값뿐이다.)
> 바뀐 절만 읽으려면: §6-0(마크업의 `#bodyHint` 자리표시 문구) · **§6-2(머리 — "짝은 단축키가 아니다" · Ctrl = Ctrl/Cmd · 잠긴 동안 발동 없음 / 표 — 짝 · 주석 토글 · 줄 복제 · 줄 이동 · 툴바 코드 블록 언어 기억 · 보조키 규칙 행 신설,
> Ctrl+B/I · Ctrl+S · Tab/Shift+Tab · `//` · ⑤ 행 정정 / 신설 소절 "코드 블록 안의 짝" · "주석 토글" · "줄 범위" · "코드 블록 언어 기억" · "알리는 자리(v4.2)" /
> Enter 규칙 표 머리·"위치와 순서"·"하지 않는 것"의 자동 닫기 문장을 "펜스 밖"으로 좁힘)** · **§12-18(작업 #150-156)** · §13.
> **v4.2 추가 확정 (2026-09-23) — §10 푸터 둘째 줄.** 사용자 원문: *"푸터 문구에 추가하는데 문장은 "본 게시글 내용은 직접 작성하여 정리했습니다."
> 이걸 문법적 오류와 설명, 가독성, 현재 구조에 맞게 푸터 메인화면에 추가해"*. 선택지 답변으로 확정: 문장 **`게시글은 모두 직접 작성하고 정리하였습니다`** ·
> 범위 **세 페이지 모두**. 처음 등재 때 미확정으로 뺐던 작업 트리의 줄(`게시글 내용은 직접 손수 작성하였습니다`)은 이 문장으로 **교체**된다.
> 클래스 신설 없음(둘째 줄도 `.footer-note`) — CSS는 `components.css`의 간격 두 값만(§10 "간격").
> 바뀐 절: **§3(셸 마크업의 푸터) · §10 · §12-18(#157-159)** · §13.
> **이행 상태(2026-09-23)**: `editor.js` 몫(#150-155)은 구현 완료(미커밋) — 이 개정이 그 동작을 명세로 삼았다. #156 ✔(frontend-dev-2).
> #157(계약·CSS) ✔ · #158(frontend-dev — index.html·post.html) ✔ · #159(frontend-dev-2 — write.html) ✔ — 세 페이지 푸터 둘째 줄이 확정 문장과 글자 일치
> (web-designer 대조). **이행 완료 — §12-18 #150-159 전부 ✔.** 단 #150-155의 실키보드·IME 입력은 미확인(§12-18 #156 비고).
...
### 0-4. 사용자 습관 목록 — 모든 검증의 필수 입력 (v4.3 신설 — meeting-08 R-C · R-H)

> v4.2까지 기능은 "글 0편" 시점의 가정(요약 = h2, 언어 = 소문자, 목록 간격 = 문단 간격, 펜스 = 툴바가 만드는 모양) 위에 설계됐고,
> 완료 판정은 "코드가 계약 문장과 같은가"였다. 픽스처를 **계약서를 읽고** 만들었으니 계약의 가정은 늘 통과했다 — 첫 실제 글이 생긴 뒤에야
> 요약 카드가 한 번도 안 뜨고(D14) `라벨:` 목록이 떨어지고(D16) ```` ```Java ````를 에디터가 못 알아보는(D18) 것이 드러났다.
> 이 표는 **글쓴이가 실제로 쓰는 모양**이다. 계약의 가정이 아니라 관찰이다.

| # | 습관 | 관찰 근거 (`posts/java/2026-09-21-note-125149.md`) | 걸리는 절 |
|---|---|---|---|
| H1 | 코드 블록 언어를 **대문자로** 친다 — ```` ```Java ```` | `:15` `:46` (소문자 0회) | §6-2 `//` · 언어 기억 · 주석 토글 · §5-7 라벨 |
| H2 | **`라벨:` 한 줄 + 바로 아래 목록** — 제목(h2)이 아니라 문단이다. 빈 줄 없이 붙여 쓴다 | `:36` `핵심 문법:` · `:62` · `:73` `핵심 정리:` | §5-7 "라벨 + 목록" · §5-7-2 트리거(U3 = c — 카드로 옮기지 않는다) |
| H3 | **느슨한 목록** — 항목 사이에 빈 줄을 둔다(같은 글 안에 촘촘한 목록도 있다) | `:74-84` | §5-7 "느슨한 목록" |
| H4 | **코드 안의 한글** — 주석 · 문자열이 한국어다(한글 IME로 코드 블록 안을 친다) | `:22` `:25` `:32` `:51` `:56` | §6-2 짝 · 조합 확정 뒤 건너뛰기 · 주석 토글 |
| H5 | **사이드바 고정** — 고정은 페이지를 넘어 이어져 에디터도 도킹된 채 열린다(1024 이상) | `theme-init.js` `blogSide`(§3-2) · meeting-08 D12 | §3-2 · §6-0 폭 규칙 · §11-3 |
| H6 | 제목이 **한글만** 이거나 **`Java …`로 시작**하고, 분류는 한두 개(`java`)에 몰린다 — 자동 id가 같은 날 겹친다 | 제목 `캡슐화` → id `note-125149`(폴백) · meeting-08 D1 | §6-1 저장 순서 · 자동 접미사 |
| H7 | 절 첫머리에 **정의 한 줄을 인용(`>`)으로** 둔다 | `:13` `:44` `:70` | §5-7 인용 · 목차(인용은 목차에 안 든다) |

**규칙 — 이 목록은 검증의 입력이다.**
- **계약 작업표(§12)의 ✔는 "이 표의 해당 행을 입력으로 돌려 결과를 봤다"는 뜻이다.** 코드와 계약 문장의 대조만으로는 ✔를 붙이지 않는다.
  보고에 "무엇을 입력으로 넣었는지"(행 번호)를 적는다.
- 검증 입력은 세 묶음이다(`/fixture` 샌드박스 사본 안에서): ① **실제 글 사본**(위 글 — 사본은 `posts/`의 원본을 그대로 쓴다. 고치지 않는다)
  ② **경계 문서** — 빈 문서 · 첫 줄이 빈 줄(`"\n…"`) · 끝 줄에 개행 없음 · 닫히지 않은 펜스 ③ **붙여 넣은 외부 마크다운** — `~~~` 펜스 · 목록 안 펜스 · 백틱 4개 펜스 안의 ```` ``` ````.
- 표의 갱신은 web-designer가 한다. 새 글에서 새 습관이 보이면 행을 더하고 근거(파일:줄)를 적는다. 습관이 바뀌어도 옛 행은 지우지 않는다 — 옛 글이 그 모양으로 남아 있다.
- 글 내용(오타 · 사실 오류)은 사용자 데이터다. 검증 중에 보여도 고치지 않는다(meeting-08 U6).
...
2. **개성은 역할이 정해진 색 셋 + 타이포그래피 위계로 낸다.** (v3.1 — 사용자 판정
   ...
   **예외는 없다(v4.4).** 제목 타자의 커서(§3-5)는 역할색이 아니라 글자색(`--c-text`)이다 — 누르는 것도 시간도 주제도 아니고, 글을 쓰는 자리다.
...
4. **모션은 상태 변화에만.** stagger·무한 루프는 없다.
   ...
   **v4.4: 첫 방문 연출**(§3-5)이 셋째다 — 세션당 한 번, index.html에서만, 사용자가 직접 요구한 두 가지:
   골격 셋이 12px 아래에서 0.6초에 떠오르고(0ms부터), 다 올라온 순간 제목이 한 글자씩 쓰인 뒤 커서가 세 번 깜빡이고 사라진다(v4.5 — 제목 6글자 기준 첫 페인트부터 4.7초, 어떤 제목이든 **첫 글자부터** 5초 미만 · §3-5 "5초 예산").
   **타자는 stagger가 아니다** — 여러 요소를 차례로 들이는 것이 아니라 한 요소(h1)의 글이 쓰이는 상태 변화 하나다. **깜빡임은 세 번으로 끝난다**(무한 루프 아님, WCAG 2.2.2).
   v3.6~v4.3의 인트로 전등(막·전구·건너뛰기)은 v4.4에 폐기됐다(§13). **테마 토글에는 어떤 애니메이션도 없다** — 아이콘의 scale·box-shadow transition도
...
모션: --ease(cubic-bezier(.16,1,.3,1))  --dur-fast(160ms) --dur(260ms) --dur-slow(420ms)
      ...
      --dur-intro(600) --dur-caret-blink(1100, v4.5 — v4.4 1000)   ← 첫 방문 연출 전용(§3-5, v4.4)
...
z-index: --z-bar(30, v4.1) --z-scrim(40) --z-side(50) --z-modal(60) --z-toast(80)    (--z-intro는 v4.4 삭제 — 막이 없다)
```
...
**v4.4 — 첫 방문 연출 토큰 둘(§3-5). `layout.css` §10 · `components.css` §16만 쓴다 — 다른 곳에서 쓰면 결함이다.**
v3.6~v4.3의 인트로 토큰 여덟(`--z-intro` `--dur-intro-in` `--delay-intro` `--dur-intro-on` `--hold-intro` `--dur-intro-exit` `--dur-intro-out` · `--c-glow`)과
파생 변수(`--intro-exit-at` `--intro-out-at` `--bulb-*`)는 전구와 함께 삭제됐다(아래 "삭제된 토큰").

| 토큰 | 값 | 왜 이 값인가 |
|---|---|---|
| `--dur-intro` | `600ms` | **두 가지를 같은 값으로 잰다.** ① 골격 셋이 12px 아래에서 떠오르는 시간(`intro-arrive`, `--ease`). expo-out이라 180ms에 87%, 300ms에 97%가 보이고 나머지는 "자리 잡는" 꼬리다 — 400 아래로 줄이면 꼬리가 사라져 "튀어나온다", 800부터는 첫 화면이 기다리게 한다(v3.x의 막 퇴장과 같은 값 — 사용자가 2.25초 동안 본 "올라오는 박자"를 그대로 둔다). ② 제목이 투명하게 기다리는 시간(`title-wait`) = **첫 글자의 시각**. 타자가 올라오기 끝에서 시작해야 두 모션이 겹치지 않고 "종이가 자리 잡는다 → 글이 쓰인다"의 두 박자로 읽힌다. 그리고 JS가 타자 DOM을 만들 여유다(§3-5 조건 1) — 스크립트 여섯이 CSS 뒤에 늦게 와도 600ms 안이면 타자가 산다 |
| `--dur-caret-blink` | ~~`1000ms`~~ → **`1100ms`(v4.5)** | 커서 깜빡임 **한 번**(켜짐 550 + 꺼짐 550). OS 입력 커서의 기본 박자(Windows 530ms씩)에 가깝게 — 더 빠르면 "경고등", 더 느리면 "멈춘 것". 횟수는 **3**(`components.css` §16, 횟수는 토큰이 아니다 — 지속시간이 아니라 수다): 3 × 1100 = 3.3초. **v4.5 — 사용자 "커서 깜빡이는 시간 살짝만 늘려"를 회수가 아니라 한 주기로 받았다.** 회수 4(× 1000 = 4초)는 "살짝"이 아니라 +33%이고, 상한 제목이면 첫 글자부터 1600 + 4000 = 5.6초로 5초 예산을 넘는다. 주기 +10%는 반 박자 550ms라 오히려 OS 커서(530)에 더 가깝다 — 1300(650씩)부터는 켜짐이 길어 "깜빡"이 아니라 "숨쉬기"로 읽힌다. 1130(3.39초)은 상한 제목의 끝이 4990ms로 안전망 5000과 겹친다. 끝 글자에서 커서가 사라질 때까지 3.3초, 첫 글자부터는 제목 6글자면 4.1초 · 타자 상한(§8-1 1600ms)이면 4.9초 — WCAG 2.2.2의 5초 안이라 멈춤 장치가 필요 없다 |

**글자 간격·타자 구간 상한은 토큰이 아니다** — JS만 쓰는 시각이라 §8-1 표의 숫자가 진실이고 `app.js`가 상수로 갖는다(토스트 3200·모달 220과 같은 자리).
CSS와 JS가 함께 알아야 하는 시각은 "첫 글자 = `title-wait`가 끝나는 순간" 하나뿐이고, 그것은 숫자가 아니라 **사건**으로 맞춘다(§3-5 — JS가 숫자를 복사하지 않는다).
...
**v3.4 — 색 토큰은 `@property`로 등록된다.** `tokens.css` 끝에서 `--c-*`(~~19~~ ~~21~~ **20** — v3.8에 `--c-bg-2` 삭제, v4.0 `--c-glow` 추가 → v4.4 삭제, v4.1 `--c-border-strong`) · `--code-*`(3) · `--hl-*`(17)을
`syntax: "<color>"`로 등록한다(합계 ~~39~~ ~~41~~ **40**). 목적은 하나 — 테마 토글 때 **토큰 값 자체가 보간**되게 하는 것(§11).
...
**v4.4 폐기(전구 인트로와 함께 — §3-5): `--z-intro` `--dur-intro-in` `--delay-intro` `--dur-intro-on` `--hold-intro` `--dur-intro-exit` `--dur-intro-out` · `--c-glow`(`@property` 등록·`base.css` 전환 목록에서도 — 등록 합계 41 → 40) ·
파생 변수 `--intro-exit-at` `--intro-out-at` `--bulb-from` `--bulb-line` `--bulb-fill` `--bulb-wire` `--bulb-cap-line` `--bulb-cap-fill` `--bulb-glow-in` `--bulb-glow-out`**
...
```html
<!-- v4.4: 세 페이지 모두 스킵 링크가 <body>의 첫 자식이다(index.html의 인트로 막은 폐기 — §3-5) -->
...
<!-- v4.0(M2): 보조 줄은 연도 하나다. "메모 블로그 · 2026"의 사이트 이름은 헤더 브랜드(index에서는 h1)가
     ...
     첫 .footer-note 문장은 요구사항 #16 원문(© 포함) 그대로다 — 손대지 않는다(§10).
     v4.2: 둘째 .footer-note는 사용자 추가 문구(요구사항 #16과 별개, §10). 순서 = #16 → 추가 문구 → .footer-meta -->
...
| 규칙 | 이유 |
|---|---|
| **`body`의 첫 자식 — 세 페이지 모두**(v4.4 — index.html의 막이 폐기돼 v3.5 이전과 같다) | 첫 Tab에서 나와야 "건너뛰기"다. 첫 방문 연출(§3-5) 중에도 첫 Tab은 스킵 링크다 — 연출은 아무것도 덮지 않고 `inert`도 없다 |
...
**저장 키 표 (v4.3 신설 — `config.js` `storageKeys` 전체. 여기 없는 키를 만들지 않는다)**

사이드바 절에 두는 이유 — 첫 페인트 전에 읽는 키(`blogSide` `blogTheme` `blogIntro`)가 셸의 일이고, 키 목록이 흩어져 있어 죽은 키(`admin`)와
사본 상수(`CODE_LANG_KEY`)가 생겼다(meeting-08 D8 · WD-7). 문자열은 `config.js` 한 곳이 진실이다. `theme-init.js`는 `config.js`보다 먼저 돌아
문자열을 직접 쓰는 **유일한 예외**다(표의 "사본" 열).

| `storageKeys.` | 값 | 저장소 | 모양 | 읽고 쓰는 곳 | 사본 |
|---|---|---|---|---|---|
| `theme` | `blogTheme` | local | `'light'` / `'dark'` | `ui.js` · `theme-init.js` | `theme-init.js` |
| `draft` | `blogDraft` | local | **접두**. 실제 키 = `Blog.store.draft.key(id)` = `'blogDraft:' + (id \|\| 'new')`, 값 `{savedAt, data}` | `store.js` `draft` · `editor.js` | — |
| `side` | `blogSide` | local | `{"pinned":bool,"closed":[slug…]}` | `ui.js` · `theme-init.js` | `theme-init.js` |
| `intro` | `blogIntro` | session | `'1'` | `ui.js` `initIntro()` 쓰기(index.html 첫 방문 로드) · `theme-init.js` 읽기 → `html[data-intro="done"]`(§3-5, v4.4) | `theme-init.js` |
| `list` | `blogList` | session | `{"search":"?…","y":n}` | `app.js` · `post.js`(§4-6) | — |
| `listReturn` | `blogListReturn` | session | 글 id | `post.js` 쓰기 · `app.js` 읽고 지움(§4-6) | — |
| **`codeLang`** (v4.3 등재) | `blogCodeLang` | local | 언어 글자(§6-2 "코드 블록 언어 기억") | `editor.js` | — |
| ~~`admin`~~ | ~~`blogAdmin`~~ | — | **v4.3 삭제.** 관리자 판정은 hostname 하나다(`admin.js` — `?admin=1`·localStorage 스위치 폐기). 읽는 곳 0 — 남겨 두면 "켜는 스위치가 있다"로 읽힌다 | — | — |

- **`Blog.store.draft.key(id)`**(v4.3 — frontend-dev) — 초안의 실제 localStorage 키를 돌려주는 export. `store.js` 안의 `draftKey()`를 그대로 내보낸다.
  `storage` 이벤트(`e.key`)로 "다른 탭이 이 글의 초안을 바꿨다"를 가려내려면 에디터가 키를 계산할 수 있어야 한다(B8-5의 선행 — 이 개정은 export만 한다).
  `editor.js`가 `'blogDraft:'`를 직접 조립하지 않는다.
- **`codeLang`의 이행 판단(v4.2 등재)은 효력을 잃는다** — v4.2는 "읽고 쓰는 파일이 하나뿐"이라 `editor.js` 상수(`CODE_LANG_KEY`)로 뒀다. 키 목록을 한 곳에 모으는 것이
  이 표의 목적이라 `CFG.storageKeys.codeLang`으로 옮긴다. `editor.js`는 `(CFG.storageKeys && CFG.storageKeys.codeLang) || 'blogCodeLang'`로 읽는다(config.js가 늦게 바뀌어도 값이 같다).
...
### 3-5. 첫 방문 연출 — 페이지 올라오기 + 제목 타자 (v4.4 전면 개정 — v3.6~v4.3의 인트로 전등은 폐기 · index.html만)

> **v4.4 사용자 원문(2026-09-25)**: *"전등 소리 추가만 해 / 소리는 인터넷 서치나 아무걸로 전등 소리만 가져와"* → 첫 방문의 자동 점등은 사용자 조작 전이라 브라우저 자동재생 정책이 소리를 막는다는 안내 →
> *"그냥 전등 빼고 자연스럽게 렌더링 되는 다른 애니메이션으로 바꿀까?"* → 제안(전구를 빼고 올라오기만 0초부터)에 *"페이지 올라오기로 ㄱㄱ"* ·
> *"그 학습 블로그라고 메인화면에 되어 있잖아 그거 한 글자씩 나오면서 마지막에 커서 깜빡거리는 것도 추가하자"* · 재확인 *"전구 자체를 빼"*.
> 사용자가 말한 "학습 블로그"는 index.html의 `h1.page-title[data-site-title]`이고 실제 문구는 `메모 블로그`다. **문구는 바꾸지 않는다.**

한 문장으로: **세션에서 index.html을 처음 여는 로드에만 — 바탕(종이)은 첫 페인트부터 그 자리에 있고, 헤더·본문·푸터가 12px 아래에서 0.6초에 떠오른다 →
다 올라온 순간 제목이 한 글자씩 쓰인다 → 끝 글자 뒤 커서가 세 번 깜빡이고 사라진다.** 제목 6글자면 4.7초에 끝나고(v4.5), 어떤 제목이든 첫 글자부터 5초를 넘지 않는다(아래 "5초 예산").
막·그림·버튼·소리 없음. 연출 중에도 모든 것이 제자리에 있고 누를 수 있다. 재방문·post.html·write.html·`prefers-reduced-motion`은 연출 없이 완성된 화면이다.
테마 토글과는 **아무 관계가 없다**.

#### 조건 — `html[data-intro]` 하나 (frontend-dev와 frontend-dev-2의 유일한 접점)

| 값 | 뜻 | 누가 · 언제 |
|---|---|---|
| (속성 없음) | 이 세션에서 아직 첫 방문 연출을 보지 않았다 — **이 로드가 index.html이면 연출을 한다** | 기본값 |
| `done` | 이 세션에서 이미 봤다 — 연출 없음 | `theme-init.js`(frontend-dev), 첫 페인트 전, `sessionStorage['blogIntro']`가 있으면 |

- **첫 페인트 뒤에는 누구도 이 속성을 붙이거나 떼지 않는다.** 페이지가 사는 동안 값이 하나라 CSS 선택자와 JS 판정이 어긋날 수 없다.
  로드 중에 `done`을 붙이면 올라오기가 그 프레임에 끊긴다(선택자가 빠져 애니메이션이 사라진다) — v4.0의 `finish()`가 붙이던 것은 폐기다.
- **`sessionStorage['blogIntro'] = '1'`은 `ui.js` `initIntro()`(frontend-dev-2)가 쓴다** — 첫 방문 연출의 페이지에서, 속성이 `done`이 아닐 때. 다음 로드부터 `theme-init.js`가 `done`을 붙인다.
  **첫 방문 연출의 페이지 = `<body>`의 직계 자식 `<main>`이 `.list-page`인 페이지(index.html)** — CSS 선택자 `body:has(> .list-page)`와 같은 표식이다.
  post.html·write.html에서는 쓰지 않는다 — 글 링크로 먼저 들어온 사람도 목록에 처음 올 때 한 번 본다(v3.6부터의 "index.html만"을 그대로 잇는다).
- reduced-motion이어도 쓴다(연출이 없었을 뿐 이 세션의 첫 방문은 지났다). 저장소가 막혀 있으면(try/catch) 매 로드가 첫 방문이 된다 — 깨지지는 않는다.
- `initIntro()`가 하는 일은 이 한 줄 쓰기가 전부다 — 속성 변경·타이머·이벤트 리스너·`inert`·포커스 이동·`getComputedStyle` 없음. `Blog.ui.initIntro` export와 `initShell()`의 호출은 그대로.
- `app.js`의 타자는 이 속성을 직접 읽지 않아도 된다 — 조건이 CSS 대기(`title-wait`)에 전부 담겨 있다(아래 ② "타자를 하는 조건" 1).

#### ① 페이지 올라오기 — CSS만 (`layout.css` §10)

| 규칙 | 내용 | 왜 |
|---|---|---|
| 대상 | `body`의 직계 `.site-header` `.site-main` `.site-footer`. `.side`·스크림·토스트 영역은 움직이지 않는다 | 사이드바는 제 슬라이드가 있다. 도킹된 채 열린 사이드바까지 따라 오르면 "틀"이 흔들린다 |
| 선택자 | `@media (prefers-reduced-motion: no-preference)` 안의 `html:not([data-intro="done"]) body:has(> .list-page) > :is(.site-header, .site-main, .site-footer)` | 조건 표 그대로 + 모션을 허락한 사람에게만(④) |
| 모션 | `@keyframes intro-arrive` — opacity 0 → 1 + `translate` 12px → 0. `--dur-intro`(600ms) + `--ease`, **delay 0**, fill **`backwards`** | 아래 넷 |
| 12px | v3.7~v4.3은 10px이었다 — 전구를 뒤따라 올라오는 딸림 동작이라 12부터는 "밀려 들어온다"고 봤다. 이제 이것이 연출의 주 동작이고, 사용자가 고른 이름이 "페이지 올라오기"다 — **올라온다는 것이 보여야** 한다. 16부터는 화면이 "날아든다" | 콘텐츠 도착(§11-4)의 3px과 네 배 떨어져 섞이지 않는다 |
| 0ms | 막이 없으므로 기다릴 것이 없다. 바탕(`--c-bg`)은 `body`의 것이라 첫 페인트부터 그대로 있다 — "종이가 먼저 깔리고 글이 떠오른다" | v3.x의 1650ms 대기는 전구가 떠날 시간이었다 |
| `backwards` | 끝나면 아무 값도 남기지 않는다(v3.x의 `both`와 다르다). `translate: 0 0`이 남으면 헤더가 fixed 자손(`.brand:focus-visible`, §3)의 기준 상자로 계속 남는다 | 끝 상태 = 원래 스타일(불투명·제자리)이라 보이는 차이는 없다 |
| 포커스 | 연출 중에도 Tab은 평소대로 들어간다(`inert` 없음). `--ease`(expo-out)라 0.18초에 87%가 보인다 — 포커스 링이 흐린 시간은 한 박자도 안 된다 | v4.0이 C4로 막았던 것은 2.25초 동안 **opacity 0**이던 본문이었다 |

#### ② 제목 타자 — JS가 글자를, CSS가 대기·커서를 (`app.js` · `layout.css` §10 · `components.css` §16)

**정적 마크업은 바뀌지 않는다**(§4 — `<h1 class="page-title" data-site-title>메모 블로그</h1>`). 타자하는 동안에만 `app.js`가 안을 이렇게 만든다
(`createElement` + `textContent` — `innerHTML` 없음. 스팬 사이에 공백 텍스트 노드를 두지 않는다 — 공백 하나가 제목 폭을 바꾼다):

```html
<h1 class="page-title" data-site-title>
  <span class="sr-only">메모 블로그</span><span class="title-type" aria-hidden="true"><span class="title-typed">메모</span><span class="title-rest"> 블로그</span></span>
</h1>
```

| 부품 | 규칙 | 왜 |
|---|---|---|
| `.sr-only` | 제목 전체. 타자 내내 그대로 | **① 스크린리더는 전체 제목을 한 번만 읽는다** — 보이는 쪽은 `aria-hidden` 안이라 글자가 옮겨 가도 접근성 트리가 바뀌지 않는다. h1의 접근성 이름은 타자 전·중·후 같은 문자열이다 |
| `.title-type` | `aria-hidden="true"`. 보이는 제목 한 줄. 자식은 `.title-typed` → `.title-rest` 둘, 이 순서 | |
| `.title-typed` | 지금까지 나타난 앞부분. 처음엔 빈 문자열 | |
| `.title-rest` | 아직 나타나지 않은 뒷부분. **`color: transparent`로 자리를 먼저 차지한다.** 처음엔 제목 전체, 끝 글자가 나오면 빈 문자열(`:empty` — 텍스트 노드도 없다) | **② 가운데 정렬 제목이 흔들리지 않는다** — `.title-typed` + `.title-rest` = 늘 제목 전체라 가운데 정렬 상자의 폭이 첫 순간부터 최종 폭이다. 글자를 붙여 나가면 한 글자마다 반 글자씩 왼쪽으로 밀린다 |
| 커서 = `.title-rest::before` | 다음 글자가 올 자리(= `.title-rest`의 시작)에 선 **세로 막대** — 폭 2px, 높이는 글자 높이, 색 `--c-text`. `position: absolute`라 **줄 폭을 차지하지 않는다**(나타나도 사라져도 제목이 움직이지 않는다). 타자 중에는 깜빡이지 않는다 | 모양 — 막대(I-beam)는 이 사이트의 입력 커서(에디터 본문 칸)와 같은 꼴이다. 블록(▋)은 터미널 흉내, 밑줄은 링크 밑줄과 겹친다. 색 — 커서는 누르는 것이 아니라 쓰는 자리라 역할색이 아니다(원칙 2). 타자 중 고정 — 실제 편집기도 치는 동안은 깜빡이지 않는다 |
| 깜빡임 | `.title-rest:empty::before`에만 `@keyframes title-caret-blink` — 켜짐 반·꺼짐 반, `--dur-caret-blink`(1100ms — v4.5) × **3회**, 끝은 꺼짐(`forwards`). 상태 클래스 없음 — **내용이 곧 상태**다(다 썼다 = 비었다) | **③ 5초 안에 멈춘다** — 3.3초. 무한히 깜빡이는 커서는 제목을 입력칸처럼 보이게 한다(원칙 1) |
| 끝 상태 | 깜빡임이 끝나면 `app.js`가 h1을 **평문**으로 되돌린다 — 정적 마크업·재방문과 같은 DOM(자식 요소 0, `textContent` = 제목). 커서는 이미 꺼져 있고 폭도 같아 보이는 변화가 없다. 깜빡임의 끝은 `.title-rest`의 `animationend`(`animationName` `title-caret-blink`, `pseudoElement` `::before`)로 온다 | 재방문과 같은 끝. 스팬이 남으면 h1의 `textContent`가 제목을 두 번 담는다 |

**시간표 — 제목 6글자(`메모 블로그`, 공백 포함) 기준. 시각 0 = 첫 스타일 계산(CSS 애니메이션의 시작)**

| 시각 | 무엇 | 누가 |
|---|---|---|
| 0 | 첫 페인트. 골격 셋 opacity 0 · 12px 아래. **제목 글자색 투명**(`title-wait` — 정적 제목이 먼저 보였다가 비워지지 않게) | CSS |
| JS `start()` (보통 수십 ms) | 타자 DOM — 제목 자리에 **커서만**, 최종 제목의 왼쪽 끝에 | `app.js` |
| 0 ~ 600 | 올라오기(`intro-arrive`) | CSS |
| **600** | `title-wait` 끝 = **첫 글자** | CSS의 사건 → `app.js` |
| 600 ~ 1400 | 한 글자씩(간격 160ms — §8-1, v4.5). 끝 글자 1400 | `app.js` |
| 1400 ~ 4700 | `.title-rest`가 비었다 → 깜빡임 3회(1100 × 3) | CSS |
| 4700 | 평문 | `app.js` |

(v4.4는 120ms · 1000 × 3이라 끝 글자 1200 · 평문 4200이었다.)

**타자를 하는 조건 — 셋 다 참일 때만. 하나라도 거짓이면 아무것도 만들지 않는다(정적 제목 그대로, 커서 없음)**

1. **제목에 CSS 대기 `title-wait`가 걸려 있고 아직 끝나지 않았다.** 이 한 조건이 "index.html · 첫 방문(`data-intro` 없음) · reduced-motion 아님 · CSS와 `:has()`가 동작함"을
   전부 담는다 — CSS 선택자가 그 조건일 때만 대기를 걸기 때문이다(같은 판정을 JS에 따로 두지 않는다 — 두 곳이면 언젠가 어긋난다). **끝났다는 것은 완성된 제목이 이미 보였다는 뜻**이다 —
   그 뒤에 비우고 다시 쓰면 "완성 → 빈칸 → 타자"로 깜빡인다. 스크립트가 늦게 온 로드는 타자 없이 완성된 제목으로 간다.
2. **탭이 보이는 상태다**(`document.visibilityState === 'visible'`). 뒤에서 열린 탭은 타이머가 늘어져 반쯤 쓰인 제목으로 기다린다 — 보러 왔을 때 완성돼 있는 편이 낫다.
3. 제목 문자열이 비어 있지 않다.

**글자** — 사용자가 보는 한 글자 단위다. 한글 자소 조합·이모지·서로게이트 쌍을 가르지 않는다. 공백도 한 글자(한 박자)다.
**간격** — 160ms로 고르게(§8-1). 사람 흉내의 불규칙한 간격은 넣지 않는다. 제목이 길어 첫 글자부터 끝 글자까지가 1600ms를 넘으면 간격을 줄여 그 안에 맞춘다 — 어떤 제목이든 전체가 5초를 넘지 않게.
v4.5 — 사용자 *"글자가 너무 빨리 나와 속도 조금만 줄이고"*. 120ms(초당 8.3자)는 "쳐지는" 것이 아니라 "쏟아지는" 속도였다 — 6글자가 0.6초, 올라오기(0.6초)와 같은 길이라 두 박자가 한 박자로 붙었다.
160ms(초당 6.25자)면 6글자가 0.8초로 올라오기보다 길어져 "자리 잡는다 → 쓰인다"가 둘로 갈린다. 200부터는 한 글자마다 기다리게 해 "조금만"을 넘는다. 상한은 같은 비율(4/3)로 1600 — 11글자까지는 간격이 줄지 않는다.

**5초 예산 (v4.5 명시)** — WCAG 2.2.2의 5초는 **첫 글자부터 평문까지**로 잰다: `상한(1600) + 깜빡임(3 × 1100 = 3300) = 4900 < 5000`. 안전망(§8-1 첫 글자 뒤 5000ms)과 같은 기점이다.
첫 페인트부터 재지 않는 이유 — 0~600ms는 페이지 올라오기라는 **다른 모션**이고 그것이 끝난 뒤에 제목의 글이 움직이기 시작한다. 한 요소의 움직임이 멈추지 않고 5초를 넘는 일은 없다.
v4.4는 "제목 6글자 기준 첫 페인트부터 4.2초"로 적었다 — 기점을 문장에 드러내지 않아 박자를 조금만 늘려도 "5초를 넘는다"로 읽혔다. 실제 제목 `메모 블로그`(6글자)는 첫 페인트부터 재도 4.7초다.
**간격·상한·깜빡임 셋 중 무엇이든 늘리려면 이 식이 5000 미만인지 먼저 본다** — 넘으면 안전망이 깜빡임 도중에 제목을 끊는다.
**글자는 즉시 나타난다** — 페이드·이동 없음(타자는 "쓰인다"이지 "들어온다"가 아니다).

**`[data-site-title]` 대입(`app.js` `fillSite()`)과의 관계** — index.json의 `site.title`은 타자가 시작된 뒤에 도착할 수 있다.
- 타자 중인 h1에는 대입하지 않는다(대입하면 타자 DOM이 통째로 지워진다).
- 도착한 제목이 타자 중인 제목과 **같으면** 아무것도 하지 않는다 — 끝나면 같은 평문이 된다.
- **다르면** 타자를 그 자리에서 끝내고(커서 없이) 받은 제목을 평문으로 넣는다. 헤더 `.brand`의 `[data-site-title]`과 `document.title`은 지금처럼 대입한다.

**JS가 실패하면 (⑤ · §8-2)** — 타자 DOM이 없다. `title-wait`는 CSS가 600ms에 스스로 끝내므로 **정적 제목이 완성된 채 보인다**(누가 보이게 하는가 = CSS). 올라오기도 CSS가 스스로 끝낸다.
**`prefers-reduced-motion: reduce` (④)** — `layout.css` §10이 올라오기와 `title-wait`를 `@media (prefers-reduced-motion: no-preference)` 안에만 둔다 — reduce면 애니메이션 이름 자체가 없다
(`base.css`의 1ms 규칙에 맡기지 않는다: 1ms짜리 대기가 남으면 조건 1이 참으로 읽힐 수 있다). 조건 1이 거짓이라 JS는 DOM을 만들지 않는다 — **첫 페인트부터 완성된 제목, 커서 없음.**
**bfcache** — 되살아난 페이지는 멈춘 곳에서 이어진다(타이머와 CSS가 함께 멈췄다 이어진다). 받아들인다 — 남은 길이가 5초보다 짧다.

**하지 않는 것** — 소리(첫 방문은 사용자 조작 전이라 자동재생 정책이 막는다 — 사용자가 듣고 뺐다) · 건너뛰기 버튼·Esc·`inert`(끝낼 것이 없다 — 첫 페인트부터 모든 것이 제자리에 있고 누를 수 있다) ·
무한 깜빡임 · 타자 뒤 커서 상주 · 다른 페이지(post·write)의 제목 타자 · 헤더 `.brand`의 타자(index.html에서는 시각적으로 숨어 있다 — §3) · 검색 placeholder 타자.
...
- 액센트 바(`.hero::before`)·통계·진입 애니메이션 없음. 제목과 검색, 둘이 전부다.
  (v4.4: 세션의 첫 로드에 한해 페이지 전체가 올라오고 제목이 한 글자씩 쓰인다 — §3-5. 머리말만의 진입 모션이 아니라 첫 방문 연출이고, 제목의 크기·자리·문구는 그대로다.)
...
- **색은 전부 토큰의 `color-mix`다.** `@property`로 등록된 색 토큰을 읽으므로 테마 크로스페이드에서 그라디언트도 프레임마다 다시 계산돼 함께 건너간다(§11).
...
| ⑧ 첫 방문 연출 | 연출이 도는 로드(§3-5 — 세션의 첫 index.html)에는 돌아올 목록이 없다(`blogList`는 index.html이 쓴다) — 규칙이 겹치지 않는다. 연출과 복원이 한 로드에 함께 참인 경우는 없다(목록을 한 번 봤으면 `data-intro="done"`) | — |
...
**v4.3 — 깨진 frontmatter의 화면 (meeting-08 D5 · meeting-07:122 지시가 작업표로 옮겨지며 빠졌던 것).**

> 원칙: **방문자는 메타 덩어리도, 파싱 경고도 보지 않는다. 경고는 고칠 수 있는 사람(관리자)에게만 간다.**
> 실측(frontend-dev): `---` 앞 빈 줄 하나로 메타 8줄이 setext h2 하나(150자 id)가 되고, 닫는 `---`가 없으면 메타가 본문 첫 문단으로 찍혔다.
> 그 위에 `frontmatter를 읽지 못해 목록 정보로 표시합니다` 경고 토스트가 **방문자에게도** 떴다 — 방문자는 고칠 수 없는 내부 사정을 들었고,
> 그 문장은 글이 index.json에 없을 때 거짓이었다(제목이 `(제목 없음)`).

| 누가 | 파싱 실패(`post.frontmatterOk === false`)일 때 보는 것 | 이번 개정 |
|---|---|---|
| **방문자** (`Blog.admin.isAdmin()` false) | 토스트 없음. 제목·날짜·분류는 index.json 항목(§9-1)으로. 본문은 아래 "메타 덩어리" 행 | **토스트를 부르지 않는다**(#163) |
| **관리자** (localhost 계열) | 경고 토스트(`warn`) 한 번 — 문구는 아래 둘 중 하나 | 문구 정정(#163) |
| 메타 덩어리(본문 맨 위에 찍힌 `id: …` 줄들) | **목표: 방문자 화면에서 사라진다.** 수단은 관용 파싱(여는 `---` 앞 빈 줄 허용 · `META_KEYS` 줄만 메타로)이고, `store.js`와 `server/posts.py`가 **같은 규칙**이어야 해서(골든 벡터 선행) 이월 백로그 **B8-12**로 간다 | 이번 개정에서는 바뀌지 않는다 — 규칙만 여기 고정한다 |

관리자 경고 문구(글자 그대로, 문장 안 나열은 쉼표 — 원칙 7):

| 조건 | 토스트 |
|---|---|
| 파싱 실패 · 글이 index.json에 있다(`store.findMeta(id)`가 값을 돌려준다) | `frontmatter를 읽지 못해 index.json의 제목과 날짜로 표시합니다. 파일 맨 위 --- 블록을 확인해 주세요` |
| 파싱 실패 · 글이 index.json에도 없다 | `frontmatter를 읽지 못했고 index.json에도 이 글이 없어 제목과 날짜를 알 수 없습니다. 파일 맨 위 --- 블록을 확인해 주세요` |

- 판정은 `render()` 안에서 `Blog.admin.isAdmin()` 한 번. 토스트를 방문자에게 숨기는 것은 **보안이 아니라 소음 제거**다(관리자 판정 자체가 UI 스위치 — `admin.js` 주석).
- `parsed.warnings`(해석할 수 없는 줄)를 화면에 올리는 일, 에디터 수정 모드에서 깨진 글을 열 때 메타가 본문 칸에 들어가는 경로(meeting-08 N-8, 미확인)는 B8-12와 함께 다룬다.
...
- **v4.3 — 라벨 + 목록(§0-4 H2 · meeting-08 D16).** 문단 바로 뒤에 목록이 오면 그 문단은 목록의 머리(`핵심 문법:`)다. 간격 셋의 위계:
  **위 덩어리 → 라벨 32/20 > 라벨 → 목록 12(`--sp-3`) > 항목 → 항목 8(`--sp-2`)**. 라벨이 위 문단·코드 블록보다 목록에 가까워야 한 덩어리로 읽히고,
  항목 사이보다는 멀어야 불릿 빠진 첫 항목으로 보이지 않는다.
  CSS(`prose.css` §4): `.prose p:has(+ ul, + ol) { margin-block-end: --sp-3 }` **와** `.prose p + ul, .prose p + ol { margin-block-start: --sp-3 }` — **둘이 짝이다.**
  형제 여백은 겹치므로(collapse) 목록 위만 줄이면 `max(20, 12) = 20`으로 아무것도 바뀌지 않는다(회의 원안은 목록 위 한 줄이었다 — 실측으로 기각).
  `:has()`를 모르는 브라우저는 v4.2 간격(20)으로 남는다. 조건은 "문단 + 목록"뿐이다 — 콜론을 CSS로 가를 수 없고, 목록 바로 앞의 문단은 거의 언제나 그 목록의 도입이다.
  **실측(실제 글 사본, 360 · 1024, 2026-09-23)**: 라벨 → 목록 20 → **12**(상자 간격, 두 폭 같음), 위 코드 블록 → 라벨 32 그대로, 항목 사이 8 그대로, 가로 넘침 없음.
- **v4.3 — 느슨한 목록(§0-4 H3).** 항목 사이에 빈 줄을 두면 marked가 항목마다 `<p>`를 넣고, 그 `p`가 `--sp-5`를 받아 항목 사이가 **20**(같은 글의 촘촘한 목록은 8)이었다.
  `.prose li > p { margin-block: 0 }` — 항목 간격은 `li`가 정한다(느슨함과 촘촘함이 같은 8). 한 항목 안의 둘째 문단부터 `.prose li > p + p { margin-block-start: --sp-3 }`.
  실측: 느슨한 목록 항목 사이 20 → **8**, 글 전체 높이 360에서 2926 → 2842px.
- **v4.3 — 이미지 경로(사용자 결정 U4 = "규칙만" · meeting-08 D27).** 이미지 업로드 기능은 만들지 않는다. 그래프·그림은 사용자가 파일을
  **`posts/<분류>/img/`** 에 직접 넣고 본문에 **`posts/<분류>/img/<파일>`** 로 쓴다(예: `![상속 구조](posts/java/img/inherit.png)`).
  본문은 `post.html`·`write.html`(둘 다 저장소 루트)에서 그려지므로 경로의 기준은 **`.md` 파일이 아니라 페이지**다.

  | 표기 | 상세(post.html) | 미리보기(write.html) | 공개 사이트(Pages 프로젝트 주소 `/<저장소>/`) |
  |---|---|---|---|
  | `posts/java/img/a.png` | 뜬다 | 뜬다 | 뜬다 — **유일하게 세 곳 모두 되는 표기** |
  | `img/a.png` (`.md` 기준 상대) | 404 | 404 | 404 |
  | `/posts/java/img/a.png` (루트 절대) | 로컬만 뜬다 | 로컬만 뜬다 | 404 — 사이트가 도메인 루트가 아니다 |

  `.md` 기준 상대 경로를 받게 하는 렌더 옵션(`renderInto(…, {assetBase})`)은 이월 백로그 B8-8이다 — 그때까지 위 표가 규칙이다.
  이미지 파일도 `posts/` 아래라 `.nojekyll`·Pages 반영 규칙이 글과 같다. 이미지 lazy·면 규칙은 아래 v4.1 등재 그대로.
...
| 트리거 문구 (v4.1) | `CFG.recap.heading`(`'다시 볼 때 이것만'`) **또는** `CFG.recap.aliases`(`['핵심 정리']`)의 어느 것과 같은 h2. 비교는 `trim()` 뒤 **끝의 콜론(`:` `：`)을 떼고** 한다(`## 핵심 정리:`도 받는다). **v4.3 정정(meeting-08 D14 · 사용자 결정 U3 = c)**: 대상은 **`h2`뿐**이다. 콜론 처리는 h2 제목 끝의 콜론을 위한 것이지, 문단 라벨(`핵심 정리:` 한 줄 + 목록 — §0-4 H2)을 받는다는 뜻이 아니다. 문단 라벨형은 카드로 옮기지 않는다(B8-10 하지 않음) — 본문 끝 제자리에서 §5-7 "라벨 + 목록" 간격으로 읽힌다 | meeting-07 M10 — 실제 글은 "핵심 정리"로 요약 절을 써서 템플릿 문구 하나만 받으면 기능이 한 번도 켜지지 않았다. 별칭은 `config.js` 한 곳에서 늘린다 |
...
| **칸 아래 오류 `.field-error`** | 제목·분류·새 분류·본문 넷에 `<p class="field-error" id="…Error" hidden>`(**v4.3: + 파일 id `#fIdError` — 다섯**). 칸에 `aria-invalid="true"` + `aria-describedby`에 오류 id 추가 | ⑥. 아래 "오류 규칙" |
...
<div class="editor" data-admin-only hidden>
  ...
  <div class="editor-head">
    ...
    <details class="editor-more" id="editorMore">
      ...
      <div class="field-group">
        ...
        <!-- v4.3(D1): 같은 id의 글이 이미 있을 때. 도움말 "뒤" — 규칙(도움말)을 먼저 읽고 무엇이 어긋났는지(오류)를 읽는다 -->
        <p class="field-error" id="fIdError" hidden></p>
      </div>
  ...
  <div class="editor-split" id="editorSplit" data-mode="split">
    <div class="editor-pane">
      ...
      <p class="sr-only" id="bodyHint">…(§6-2 "알리는 자리(v4.2)"의 문장 — 글자 그대로)…</p>
...
| **`#fId`** (v4.3 — D1) | **`#fIdError`** | 저장 때 **손으로 친 id**(또는 수정 중 바꾼 id)의 글이 이미 있다 — 에디터 사전 확인 또는 서버 `409 exists`(§6-1 "저장 순서") | 새 글: `이 id의 글이 이미 있습니다(<경로>). 다른 id를 적거나 칸을 비워 두세요. 비우면 저장할 때 새로 만듭니다.` · 수정 중 id를 바꿨을 때: `이 id의 글이 이미 있습니다(<경로>). 다른 id를 적거나 원래 id(<원래 id>)로 되돌려 주세요.` · 자동 id의 접미사 `-2`부터 `-9`까지가 전부 찼을 때: `같은 날 같은 제목의 글이 많아 id를 정하지 못했어요. 파일 id를 직접 적어 주세요.` |
...
- **토스트로 말하지 않는다** — 위 넷(v4.3: 다섯)은 `U.toast`를 부르지 않는다. 토스트는 칸과 무관한 결과(저장 성공, 서버 오류, 분류를 만들었다)만 말한다.
...
- **v4.3 — `#fId`의 오류는 `validate()`의 셋 다음이다.** 순서 제목 → 분류 → 본문 → **id 존재 확인**(§6-1 "저장 순서" ②). 앞의 셋은 폼만 보면 되지만 id 확인은 index(와 서버)를 봐야 해서
  검증이 모두 통과한 뒤에만 한다. `<경로>`는 서버 응답의 `error.path`, 사전 확인이면 index 항목의 `posts/<category>/<id>.md`.
  `error.path`가 `null`이면(index에만 있고 파일은 없다 — api.md v1.2 §3-1) 괄호 안을 **`index.json에만 있음`** 으로 쓴다. 다른 추가 필드(`title` · `via`)는 문구에 쓰지 않는다 — 문구가 길어지면 칸 아래 한 줄이 넘친다.
  **지우기**: `#fId`의 `input` 때(다른 셋과 같다). `#fId`의 `aria-describedby`는 `fIdHint`를 지우지 않고 `fIdError`를 더한다.
- **v4.3 예외 — 접힌 `.editor-more` 안 칸의 오류는 "펼친 뒤" 포커스한다.** `showFieldError(dom.id, dom.idErr, text)`는 포커스 **전에** `#editorMore.open = true`.
  닫힌 `<details>` 안의 칸은 포커스를 받지 못하고(보이지 않는 칸 — 포커스가 `body`로 샌다) 오류 줄도 보이지 않는다.
  §6-0 "작업 줄의 JS"의 "`details`의 `open`은 건드리지 않는다(사용자가 연 것은 사용자가 닫는다)"의 **유일한 예외**이고, **여는 것만** 한다 —
  오류가 지워져도 다시 접지 않는다(사용자가 방금 그 칸을 고치는 중이다). 다른 칸의 오류로 세부 설정을 열지 않는다.
...
| **세부 설정 요약** | `syncActionButtons()`가 `#btnDelete`를 보일 때 `#editorMoreSummary` = `파일 id와 글 삭제`, 감출 때 `파일 id`. `details`의 `open`은 건드리지 않는다(사용자가 연 것은 사용자가 닫는다). **예외 하나(v4.3)**: `#fIdError`를 보일 때 여는 것만 — 위 "오류 규칙" |
...
| 저장 피드백 | **새 부품 없음.** 성공: 상태줄 `저장됨: posts/<slug>/<id>.md`(v4.0 M2 — 가운뎃점 대신 쌍점. `setStatus`, dirty 해제) + `onSaved()`가 `store.loadIndex(true)` **`store.loadCategories(true)` + `drawSide()` 재호출**(v3.9). 실패(4xx/5xx): 토스트(`is-err`)에 `error.message` 그대로, 상태줄 `.is-dirty` 유지 — **v4.3: 409는 `error.code`별로 갈린다(아래 "저장 순서와 409")**. 네트워크 실패: 토스트 `서버가 꺼졌습니다. start.bat(Docker)를 실행한 뒤 다시 연결하세요` + **서버 없음 상태로 전환**(위 표) | 상태줄·토스트가 이미 "결과를 말하는 자리"다(§8-1). 초안은 localStorage 임시저장이 붙들고 있고 `beforeunload`가 남아 있어 창을 닫으려 하면 경고한다 — 서버가 없어도 글을 잃는 길은 없다 |
| **상태줄 문구(v4.1 등재 · v4.3 보충)** | `준비 중…`(마크업) · **`불러오는 중…`**(수정 모드 fetch, 폼 잠김) · `임시저장 중…` · `임시저장됨. 아직 서버에 저장하지 않았어요` · `새 글. 아직 저장하지 않았어요`(`.is-new`) · `서버에 저장하는 중…` · `저장됨: posts/…` · **`서버에서 삭제하는 중…`**(삭제, 폼 잠김) · Tab 안내 둘(§6-2) · **v4.3 등재(이미 코드에 있던 것)**: `저장하지 못했어요. 임시저장본은 그대로 있습니다` · `서버 연결이 끊겼어요. 임시저장본은 그대로 있습니다` · **v4.3 신설**: `저장하지 않았어요. 임시저장본은 그대로 있습니다`(`stale` 확인 모달에서 `취소`) | 한 자리가 말하는 문장은 여기가 목록이다. 새 문장을 만들면 이 줄에 먼저 적는다 |
...
#### 저장 순서와 409 (v4.3 — meeting-08 D1 · 판정 충돌 2 · 충돌 10, `docs/api.md` v1.2)

> **v4.2까지의 결함**: 자동 id는 한글을 지운다(`util.slugAscii`) — 같은 날 `Java 정리`·`Java 복습`·`Java: 상속`이 모두 `YYYY-MM-DD-java`다(§0-4 H6).
> 에디터는 저장 전에 존재를 확인하지 않았고 서버는 upsert였다 → 새 글이 옛 글을 **소리 없이** 덮고, 옛 글의 `created`를 물려받고(절대 규칙 4 위반 경로),
> 토스트는 `저장했습니다` 한 단어 차이였다. 응답의 `isNew:false`는 버려졌다.
> **결정**: 서버 409가 필수 방어선이고, 에디터 사전 확인은 보조다 — 단 보조 확인은 **분류 PUT보다 먼저** 한다(409가 분류 PUT 뒤에 나면 빈 분류가 남는다).
> 사용자가 짓지 않은 이름(자동 id)은 에디터가 조용히 비키되 **알린다**. 사용자가 친 이름(손 id)은 에디터가 바꾸지 않고 **칸에서 묻는다**.

**용어** — **새 글** = 디스크에 없는 글(`!state.onDisk` — 새 글 모드이고 아직 한 번도 저장하지 않았다). **자동 id** = 새 글이고 사용자가 id 칸을 건드리지 않았다(`!state.idTouched`).
**손 id** = 그 밖(사용자가 친 id, 또는 수정 중에 바꾼 id).

**순서(`saveToServer()` → `doSave()`)** — 앞 단계가 멈추면 뒤 단계는 없다.

| # | 단계 | 규칙 |
|---|---|---|
| ① | 폼 검증 | `validate()` — 제목 → 분류 → 본문(§6-0 "오류 규칙", v4.1 그대로) |
| ② | **id 존재 사전 확인** (v4.3) | 대상: **새 글**이거나 **수정 중 id를 바꿨을 때**(`state.originalId && meta.id !== state.originalId`). 이미 디스크에 있는 같은 id를 다시 저장하는 평범한 수정은 대상이 아니다. `store.loadIndex(true)`(다른 탭이 방금 저장했을 수 있다 — 캐시가 아니라 새로) 뒤 `store.findMeta(meta.id)`. **index를 못 읽으면 이 단계를 건너뛴다** — 서버 409가 막는다. 있으면: **자동 id** → 아래 "자동 접미사"로 id를 바꾸고 계속 / **손 id** → `#fIdError`(§6-0) + 멈춤 |
| ③ | 분류 · 게시일 확인 | `ensureUsableCategory()` → `ensureCreated()` (v3.9 그대로) |
| ④ | 새 분류 PUT | 있을 때만, 글보다 먼저(v3.9 그대로) |
| ⑤ | 글 PUT | 본문에 선택 필드: **새 글이면 `ifNew: true`** · **디스크의 글이면 `expectedUpdated: <이 화면이 아는 디스크 updated>`** · 수정 중 id를 바꿨으면 `previousId`(v3.9 그대로). `expectedUpdated`의 값은 불러올 때 읽은 `updated`, 저장에 성공하면 응답 `meta.updated`로 갈아 둔다(`state.originalUpdated` — 새 상태 값) |
| ⑥ | 응답 | 아래 표 |

**자동 접미사** — 원래 id 뒤에 `-2`, `-3` … `-9`를 붙여 index에 없는 첫 값을 쓴다(`2026-09-23-java` → `2026-09-23-java-2`). `#fId`의 값도 그것으로 바꾼다
(사용자가 세부 설정을 열면 실제 파일 이름이 보인다). `-9`까지 전부 있으면 `#fIdError` 셋째 문구로 멈춘다 — 같은 날 같은 제목이 열 편이면 이름을 사람이 지을 때다.
접미사는 `util.slugAscii`의 규칙(영문 소문자·숫자·하이픈) 안이라 `isSafeId`를 통과한다.

**응답별 화면** — 문구는 글자 그대로. 서버 코드 이름은 api.md v1.2가 진실이다(다르면 api.md를 따르고 이 표의 "코드" 열만 고친다).

| 응답 | 조건 | 화면 | 상태줄 |
|---|---|---|---|
| `200` | 자동 접미사를 쓰지 않았다 | v4.1 그대로 — 토스트 `ok` `새 글을 저장했습니다` / `저장했습니다` | `저장됨: …` |
| `200` | ②나 아래 `exists`에서 **자동 접미사를 썼다** | 토스트 `ok` **`같은 id의 글이 있어 새 id로 저장했습니다: <새 id>`** (`새 글을 저장했습니다`를 대신한다 — 토스트 하나) | `저장됨: posts/<분류>/<새 id>.md…` |
| `200` + **`isNew: false`** | **`ifNew: true`를 보냈는데** 서버가 기존 파일을 고쳤다고 답했다 — 서버가 `ifNew`를 모르는 옛 버전(v1.1)일 때만 생긴다. 이중 안전장치 | **확인 모달**(기존 `Blog.ui.modal`, 버튼 `확인` 하나): 제목 **`같은 id의 글을 덮어썼어요`** · 본문 1행 **`<경로>에 있던 글이 지금 쓴 글로 바뀌었습니다. 저장 서버가 새 글 확인을 모르는 옛 버전입니다.`** · 2행 — `json.git.committed`면 **`옛 글이 커밋돼 있었다면 커밋 <hash 7자> 이전 이력에서 되찾을 수 있습니다.`**, 아니면 **`자동 커밋이 없어 이 화면에서는 되돌릴 수 없습니다. 백업이나 git 이력을 확인해 주세요.`** 토스트는 띄우지 않는다(모달이 결과를 말한다) | `저장됨: …`(파일은 실제로 써졌다) |
| `409` `exists` | 서버가 같은 id를 찾았다(index에 없는 손 넣은 파일, 경쟁) | **자동 id** → 자동 접미사로 id를 바꿔 **글 PUT만** 다시(분류는 이미 저장됐다), `-9`까지 / **손 id** → `#fIdError`(`<경로>` = `error.path`), 토스트 없음 | `저장하지 못했어요. 임시저장본은 그대로 있습니다`(`.is-dirty`) |
| `409` `stale` | `expectedUpdated`가 디스크 `updated`와 다르다 — 이 화면에서 연 뒤 다른 탭·편집기·손이 그 글을 저장했다 | **확인 모달**: 제목 **`다른 곳에서 먼저 저장된 글이에요`** · 본문 **`이 화면에서 연 뒤에 다른 탭이나 편집기에서 이 글이 저장됐습니다. 지금 화면의 내용으로 덮어쓰면 그쪽에서 고친 내용은 사라집니다.`** · 버튼 **`덮어쓰기`**(`danger`) · **`취소`**(ghost — 초기 포커스, 모달 규칙대로 danger는 포커스 후보가 아니다). `덮어쓰기` → 같은 글 PUT을 `expectedUpdated` **없이** 한 번 더(`previousId`가 있었으면 그대로). `취소`·Esc·✕ → 아무것도 보내지 않는다. **`error.currentUpdated`가 `null`이면**(그 글이 다른 곳에서 지워지거나 옮겨졌다 — api.md v1.2 §3-1) 제목 **`다른 곳에서 지워진 글이에요`** · 본문 **`이 화면에서 연 뒤에 이 글이 다른 곳에서 지워지거나 옮겨졌습니다. 지금 화면의 내용으로 다시 만들 수 있습니다.`** · 버튼 **`다시 만들기`**(`primary`) · **`취소`**(ghost) — 동작은 `덮어쓰기`와 같다(지울 남의 내용이 없으니 danger가 아니다) | `취소`면 `저장하지 않았어요. 임시저장본은 그대로 있습니다`(`.is-dirty`) |
| `409` `conflict` (또는 `code` 없는 409 — v1.1 서버) | 같은 id가 여러 폴더 · 대소문자만 다른 파일 · JSON 깨짐(api.md §2) | 토스트 `err`: `error.message` + **`. 파일을 손으로 정리한 뒤 다시 시도해 주세요`** — 이 꼬리는 **`conflict`에만** 붙는다(`exists`·`stale`은 사람이 파일을 만질 일이 아니다) | `저장하지 못했어요. 임시저장본은 그대로 있습니다` |
| 그 밖 4xx/5xx | — | 토스트 `err`: `error.message` 그대로(꼬리 없음) | 같음 |
| 네트워크 실패 | — | v3.9 그대로(서버 없음 상태로 전환) | `서버 연결이 끊겼어요. 임시저장본은 그대로 있습니다` |

- **`apiErrorMessage(res, json, what)`는 `json.error.code`로 가른다** — 409에 무조건 붙던 "손으로 정리" 꼬리는 `conflict`(와 코드 없는 409)에만.
  `exists`·`stale`은 `apiErrorMessage`까지 오지 않고 위 표의 갈래가 먼저 받는다.
- **분류 PUT 뒤의 `exists`로 빈 분류가 남는 경우**는 ②가 거른 뒤에도 남는 드문 경쟁(index에 없는 파일)뿐이다 — 받아들인다. 글 0편 분류는 정상 상태다(§3-2 `.is-empty`).
- `stale` 모달의 `덮어쓰기`는 **사용자가 겨냥해서 누르는** 유일한 덮어쓰기 경로다. 에디터가 스스로 `expectedUpdated`를 빼고 다시 보내는 일은 없다.
- 저장 중 id가 바뀌면(자동 접미사) `onSaved()`의 슬롯·URL 규칙(v4.0 T4-2)이 새 id를 따른다 — 추가 규칙 없음.
...
### 6-2. 단축 입력 (v3.8 신설 · **v3.9 Enter 규칙 표** · **v4.2 코드 블록 편의** — 본문 `#fBody` 안에서)
...
**v4.2의 "짝"은 단축키가 아니다** — 펜스 안에서 친 괄호·따옴표 글자는 그대로 들어가고 짝 하나를 더할 뿐이라, 글자를 빼앗지 않는다).
~~한글 조합 중(`isComposing` 또는 `keyCode === 229`)에는 어느 것도 발동하지 않는다.~~ **v4.3 정정(meeting-08 D4)**: 한글 조합 중(`isComposing` 또는 `keyCode === 229`)의 `keydown`은
**가로채지 않는다**(`preventDefault` 없음 — 조합 중인 글자를 깨뜨리지 않는다). 단 **하나의 사후 보정**이 있다 — 조합을 끝내는 키가 닫는 짝이었고 펜스 안이면, 그 글자가
들어온 **직후의 `input` 한 번**에서 건너뛰기와 같은 결과로 맞춘다(아래 "조합 확정 뒤 건너뛰기"). 폼이 잠긴 동안(`state.locked` · `readOnly`)에는 보정도 포함해 발동하지 않는다.
**"펜스 안"은 이 절 전체에서 `Blog.markdown.codeRanges`의 판정 하나다**(아래 "코드 블록 안 = `codeRanges`") — `//` · 짝 · Ctrl+/ · Enter ①②③⑤ · 툴바 코드 블록 · 언어 기억이 같은 답을 쓴다.
"Ctrl"은 macOS의 Cmd를 함께 뜻한다(`ctrlKey` 또는 `metaKey`) — 화면 문구는 `Ctrl`만 적는다(아래 "알리는 자리").

| 입력 | 결과 | 어디서 알리나 |
|---|---|---|
| Ctrl+B / Ctrl+I | 선택 영역을 `**` / `*`로 감싼다(이미 감싸여 있으면 푼다). **v4.2: Ctrl·Cmd 가운데 하나만, Alt·Shift 없이** — 아래 "보조키 규칙" 행 | 툴바 `title`·`aria-keyshortcuts`, `#bodyHint` |
| Ctrl+S | 연결됨이면 저장, 아니면 안내 토스트(§6-1). **모드별 문구 없음** — 어디서나 `Ctrl+S 저장`. 문서 전체 `keydown`에서 잡는다(본문 밖에서도) — v4.2 "보조키 규칙"의 대상이 아니다(v3.9 그대로 — Alt·Shift 조건을 더하지 않는다). **v4.3(D21): 글자 판정은 보조키 규칙과 같다** — `keyIs(e, 's')`(`e.key`가 라틴 글자가 아니면 `e.code === 'KeyS'`로 한 번 더). 한글 자판에서 `e.key`가 `ㄴ`·`Process`로 오면 v4.2는 브라우저 "페이지 저장"으로 샜다 — 가장 위험한 키가 가장 먼저 새던 자리 | `#btnSave` `title`, `#bodyHint`, placeholder |
| Tab / Shift+Tab | **선택이 없으면**(`selectionStart === selectionEnd`) Tab은 커서 자리에 공백 2칸, Shift+Tab은 **현재 줄** 첫머리에서 공백 최대 2칸 제거(v3.8 그대로 — 줄 단위). **v4.2 명시: 선택 없는 Shift+Tab 뒤에도 선택은 없고, 커서는 같은 글자 앞에 남는다**(지운 칸 수만큼 왼쪽으로. 커서가 지운 공백 안에 있었으면 줄 첫머리). 첫머리에 공백이 없으면 아무것도 바꾸지 않는다(탭 문자는 지우지 않는다). **선택이 있으면 — 한 줄이든 여러 줄이든 — 선택이 걸친 줄마다** 줄 첫머리에 2칸 넣기 / 빼기, ~~선택은 그 줄들 전체로 유지~~ **v4.3(D19): 선택은 원래 선택을 편집에 따라 옮긴 것**(아래 "편집 뒤 선택" — 들여쓴 만큼 옮길 뿐 넓히지 않는다. `bar`만 골라 Tab이면 `bar`가 선택된 채 남는다. v4.2까지는 줄 전체가 선택돼, 이어서 한 글자 치면 줄이 통째로 사라졌다 — v4.2 #153이 선택 없는 경우만 고친 같은 덫). "걸친 줄"은 아래 "줄 범위"(v4.2 — 다음 줄 0열에서 끝난 선택은 그 줄을 뺀다). 바뀌는 글자가 없으면(빼기인데 모든 줄에 공백이 없다) 글도 선택도 건드리지 않는다. **Esc 뒤 Tab** = 포커스 이동. **v4.2: Ctrl·Alt·Cmd가 붙은 Tab은 가로채지 않는다**(탭 전환 등 브라우저·OS 몫 — Shift만 붙은 것이 Shift+Tab이다) | placeholder, `#bodyHint`, 첫 Tab 때 상태줄 |
| **`//` 줄 첫머리 (v3.8, U-1)** | 그 줄이 ```` ``` ```` 펜스 블록이 되고 커서는 블록 안 빈 줄. `//java`처럼 언어를 붙이고 **Enter 또는 Space**면 ```` ```java ````. **이미 펜스 블록 안이면 발동하지 않는다**(~~커서 위쪽의 펜스 줄 수가 홀수면 안~~ **v4.3: 그 줄이 `codeAt`의 코드 줄이면** — 아래 "코드 블록 안 = `codeRanges`") — 코드 안의 `//` 주석을 지킨다. **v4.2: 언어를 붙였으면 그 언어를 기억한다**(아래 "코드 블록 언어 기억"). **v4.3(D18): 언어 글자는 대소문자를 가리지 않고 받고, 친 그대로 쓴다** — `//Java` + Enter → ```` ```Java ````(소문자로 바꾸지 않는다 — §0-4 H1, 글쓴이가 손으로 치는 모양과 같게). 글자 집합 `[A-Za-z0-9+#-]{1,20}` | 툴바 "코드블록" `aria-label`에 "(// 로도)" 병기, placeholder에 `// 로 코드 블록` 한 줄, `#bodyHint` |
| **툴바 `코드 블록` (v4.2 정정)** | **기억한 언어**(없으면 `java`)로 펜스를 연다: ```` ```<언어> ```` + 선택한 글(없으면 빈 줄) + ```` ``` ````. 넣은 뒤 **언어 글자가 선택돼 있다** — 그대로 두면 그 언어, 치면 덮어쓴다. 커서가 줄 중간이면 앞에 줄바꿈 하나를 둔다(펜스는 줄 첫머리여야 펜스다 — 빈 줄까지는 두지 않는다). v4.1까지의 ```` ```js ```` + 자리표시 `// 코드`는 폐기(언어를 바꾸면 틀린 주석이 되고 쓰기 전에 지워야 했다). **v4.3(D3 · FD2-5 · FD2-13)**: ① **선택의 시작이나 끝이 코드 블록 안(`codeAt`)이거나 선택이 펜스 줄을 걸치면 넣지 않는다** — 토스트(기본 종류) **`이미 코드 블록 안이에요. 블록 밖에서 눌러 주세요`**, 글·선택은 그대로. v4.2는 넣어서 바깥 블록을 닫고, 원래 닫는 펜스가 고아가 되어 **그 아래 문서 전체가 코드**가 됐다. ② 선택이 다음 줄 0열에서 끝나면(줄 단위 선택 — "줄 범위"와 같은 모양) 선택 끝의 줄바꿈 하나는 코드에 넣지 않고 닫는 펜스 뒤로 보낸다 — 코드 끝에 빈 줄이 생기지 않는다 | 결과가 말한다 — 선택된 언어 글자가 곧 안내다. 버튼 문구·`aria-label`은 그대로(아래 "알리는 자리"). 넣지 않을 때는 토스트 |
| **Enter — 펜스 안 (v3.9)** | 아래 "Enter 규칙 표" ①②③ | `#bodyHint` 한 문장 |
| **Enter — 펜스 밖, 목록·인용 줄 (v3.9)** | 아래 "Enter 규칙 표" ④ | `#bodyHint` 같은 문장 |
| **`}` `)` `]` — 펜스 안 (v3.9)** | 아래 "Enter 규칙 표" ⑤. **v4.2: 바로 뒤 글자가 같은 닫는 괄호면 ⑤보다 "건너뛰기"가 먼저다**(아래 "코드 블록 안의 짝") | (없음 — IDE 관행) |
| **짝 — 펜스 안 (v4.2)** | `(` `[` `{` `"` `'` `` ` ``를 치면 닫는 짝이 함께 들어가고 커서는 사이. 선택이 있으면 감싼다. 닫는 글자를 칠 때 바로 뒤가 같은 글자면 넣지 않고 넘어간다(건너뛰기). 빈 쌍 사이 Backspace는 둘 다 지운다(쌍 지우기). **펜스 밖에서는 하지 않는다.** 조건 전부는 아래 "코드 블록 안의 짝" | `#bodyHint` 한 문장 + 결과가 눈앞에 보인다 |
| **Ctrl+/ — 펜스 안 (v4.2)** | 걸친 줄을 주석으로 만들거나 푼다(토글). 접두는 **그 코드 블록을 연 펜스 줄의 언어**(v4.3: `codeRanges`의 `lang`)로 정한다. **펜스 밖에서는 아무것도 막지 않는다**(브라우저 기본). 표는 아래 "주석 토글". **v4.3(D20): 표에 없는 언어·언어 없는 블록은 아무것도 쓰지 않고 토스트로 이유를 말한다**(키는 먹는다) — 조용히 틀린 주석(`// Get-Item`)을 쓰지 않는다 | `#bodyHint` |
| **Ctrl+D (v4.2)** | 걸친 줄들을 바로 아래에 복제하고, 커서·선택도 복제본으로 옮긴다. **본문 어디서나**(펜스 안팎 무관). 브라우저의 "북마크 추가"를 막는다 | `#bodyHint` |
| **Alt+Shift+↑ / ↓ (v4.2)** | 걸친 줄들을 위·아래 줄과 맞바꾼다. 선택은 줄과 함께 움직인다. **본문 어디서나.** 문서 첫 줄에서 위로·끝 줄에서 아래로는 아무것도 바꾸지 않되 키는 먹는다(Shift+화살표의 선택 확장으로 새지 않게). Ctrl·Cmd가 붙으면 발동하지 않는다 | `#bodyHint` |
| **보조키 규칙 (v4.2)** | 본문의 Ctrl 조합(B · I · D · /)은 **Ctrl·Cmd 가운데 정확히 하나 + Alt 없음 + Shift 없음**일 때만 발동한다 — Ctrl+Shift+B(북마크 바) 같은 브라우저 조합을 빼앗지 않는다. 글자 판정은 `e.key`로 하되, `e.key`가 라틴 글자가 아니면(한글 자판에서 `ㅇ` 등) 물리 키 `e.code`로 한 번 더 본다. `/`는 `e.key === '/'` 또는 `e.code`가 `Slash`·`NumpadDivide` | (알릴 것 없음 — 브라우저 기본을 지키는 규칙) |
| **Shift+Enter** | **항상 브라우저 기본**(줄바꿈 하나, 규칙 없음). 펜스 안이든 목록 줄이든 자동 들여쓰기·접두사를 원하지 않을 때의 탈출구 | `#bodyHint` |

**전부 `replaceRange`(`execCommand`) 한 번이다 — Ctrl+Z 한 번에 한 동작이 돌아간다**(Enter 규칙 표 아래 첫 항목과 같은 근거). 건너뛰기는 글을 바꾸지 않으므로
되돌릴 것이 없다. 줄 편집(주석·복제·이동·Tab)은 바뀐 글자가 없으면 `replaceRange`를 부르지 않는다 — 빈 되돌리기 단계와 헛 dirty를 만들지 않는다.

#### 줄 범위 (v4.2 — 주석 토글 · 줄 복제 · 줄 이동 · 선택 있는 Tab이 함께 쓴다, `lineSpan()`)

선택이 걸친 줄들 = `[첫 줄의 시작, 끝 줄의 끝)`(끝 줄의 줄바꿈은 빼고). **선택이 다음 줄의 0열에서 끝나면 그 줄은 걸친 줄이 아니다** — Shift+↓로 줄을
고른 흔한 모양이고 IDE 관행이다. 선택이 없으면 커서가 있는 줄 하나.

#### 줄 시작 (v4.3 — meeting-08 D17, `lineStartAt(value, p)` · editor.js 내부 함수 하나)

**정의(동작 문장)**: 위치 `p`의 줄 시작은 **`p`보다 앞(`0`부터 `p − 1`까지)에 있는 마지막 줄바꿈 바로 다음 자리**다. 그런 줄바꿈이 없으면 **`0`**이다.
따라서 `p = 0`이면 줄 시작은 언제나 `0`이고, 줄 시작은 **절대 `p`보다 뒤가 아니다.**

- 이 문장이 v3.9 Enter 표의 공식 `value.lastIndexOf('\n', pos - 1) + 1`을 **대체한다.** 그 공식은 `pos = 0`에서 틀린다 — JS의 `lastIndexOf`는 음수 시작 위치를 `0`으로 잘라
  0번 글자를 검사하므로, 문서가 `\n`으로 시작하면 줄 시작이 `1`(커서보다 뒤)이 됐다. 계약이 구현 공식을 규칙으로 적어 코드 결함이 계약으로 역수입된 자리다(meeting-08 R-C).
- 실측된 증상(frontend-dev-2): 문서 `"\n  foo"` · 커서 0에서 Shift+Tab → **커서가 없는 둘째 줄**의 들여쓰기가 지워짐 / 툴바 H2 → `IndexSizeError` / Alt+Shift+↑↓ → 줄은 안 움직이고
  빈 줄이 생김 / 계기판 `1행 0열`.
- **줄 시작이 필요한 곳은 전부 `lineStartAt`을 부른다**(지금 8곳: `updateCaret` · `linePrefix` · `learnFenceLang` · `maybeFence` · `lineAtCaret` · `lineSpan` · `moveLines`(위 줄 —
  `lineStartAt(value, span.start - 1)`) · Shift+Tab). 같은 관용구를 새로 복사하지 않는다. 이름·위치는 `editor.js` 내부(export 없음).
- **경계 문서 셋**(§0-4 ②)이 이 규칙의 합격 입력이다: `""` · `"\nabc"`(커서 0 · 1 · 끝) · `"abc"`(끝 줄 개행 없음).

#### 편집 뒤 선택 (v4.3 — D19. Tab · Shift+Tab(선택 있음) · 주석 토글이 같은 규칙, `applyLineEdits()`의 `map`)

편집 뒤의 선택은 **원래 선택의 두 끝을 각각 편집에 따라 옮긴 것**이다. 넓히지도 줄 전체로 바꾸지도 않는다.
위치 `p`와 편집(자리 `at`에서 `del`자를 지우고 `ins`를 넣음)에 대해: `p < at`이면 그대로 · `p ≥ at + del`이면 `ins.length − del`만큼 옮김(**삽입 자리와 같은 `p`는 삽입 글자 뒤로 밀린다**) ·
지워진 글자 안(`at < p < at + del`)이면 그 편집 바로 뒤(`at + ins.length`). 한 줄에 편집이 여럿이면 앞의 것부터 누적한다 — v4.2 `mapCol()`이 이미 이 계산이다.
**선택 방향(`selectionDirection`) 보존은 이 개정의 범위 밖**(meeting-08 FD2-6 → 이월 B8-13).

#### 코드 블록 안 = `Blog.markdown.codeRanges` (v4.3 — meeting-08 D3 · 판정 충돌 1)

> v4.2까지 "펜스 안"은 **커서 위쪽의 줄 첫머리 ```` ``` ```` 줄 수가 홀수**였다(`insideFence`). 근거는 "툴바가 만드는 문법만 센다"(생산자 기준)였다.
> 그러나 본문은 사람이 치고 **붙여 넣는** 글이고, 독자가 무엇을 코드로 보는지는 툴바가 아니라 **렌더러(marked 12.0.2)** 가 정한다. 대조 8개 중 6개가 갈렸고(frontend-dev 실측),
> 코드 안 `//` + Enter가 펜스 3줄을 넣어 렌더를 깨고, 줄 첫머리 인라인 ```` ```x``` ```` 하나가 **그 아래 문서 전체의 안/밖을 뒤집었다.**
> **결정**: 판정의 집은 하나 — `markdown.js`(frontend-dev)의 스캐너. 계약은 **방법이 아니라 합격 기준**을 고정한다(방법을 규약으로 삼으면 R-C를 반복한다).

**접점 (frontend-dev가 export, frontend-dev-2가 쓴다)** — 이름·모양 그대로.

```js
Blog.markdown.codeRanges(text) -> Array<{ open, bodyStart, bodyEnd, close, lang }>
Blog.markdown.codeAt(text, pos) -> range | null      // range = 위 배열의 원소
Blog.markdown.langLabel(lang)   -> string
```

| 필드 | 뜻 (오프셋은 전부 `text` 기준 **문자 인덱스**, UTF-16 — `String.prototype.slice`와 같은 단위) |
|---|---|
| `open` | 여는 펜스 **줄의 시작**(들여쓰기 포함 — 목록 안 펜스면 줄 첫 글자) |
| `bodyStart` | 여는 펜스 줄 **다음 줄의 시작**. 여는 줄이 문서 끝이면 `text.length` |
| `bodyEnd` | 닫는 펜스 **줄의 시작**. 코드는 `[bodyStart, bodyEnd)` — 마지막 코드 줄의 줄바꿈까지 들어간다. **닫는 펜스가 없으면 `text.length`** |
| `close` | 닫는 펜스 **줄의 끝**(줄바꿈 앞). **닫는 펜스가 없으면 `close = bodyEnd = text.length`** — 그래서 `close === bodyEnd` ⇔ 닫히지 않은 블록(닫는 줄은 펜스 글자 3개 이상이라 닫힌 블록은 늘 `close > bodyEnd`) |
| `lang` | 정보 문자열의 **첫 낱말, 친 그대로**(```` ```Java title="a" ```` → `'Java'`). 없으면 `''`. 비교하는 쪽이 소문자로 바꾼다(D18 — 표기는 보존, 비교만 소문자) |

- 배열은 `open` 오름차순이고 서로 겹치지 않는다. 규칙은 CommonMark 펜스 코드 블록: ```` ``` ```` 또는 `~~~` 3개 이상 · 들여쓰기 0~3칸(목록 항목 안이면 항목 내용 기준) ·
  닫는 펜스는 **같은 글자, 여는 길이 이상**, 정보 문자열 없음 · 백틱 펜스의 정보 문자열에는 백틱이 없다(그러면 펜스가 아니라 인라인 코드) · 컨테이너(목록·인용)가 끝나면 블록도 닫힌다.
- **들여쓴 코드 블록(4칸)은 대상이 아니다** — 펜스만 센다. 에디터의 입력 규칙은 "펜스가 연 블록"에 걸린다. 4칸 들여쓰기는 목록 안 둘째 문단·Tab 들여쓰기와 모양이 같아,
  그것까지 코드로 보면 목록을 쓰는 중에 짝·Enter 규칙이 켜졌다 꺼졌다 한다.
- **marked 없이 동작한다**(순수 함수, DOM·CDN 비의존) — CDN이 막히거나 오프라인이어도 에디터 입력 규칙은 산다. 같은 `text`면 마지막 결과를 돌려준다(메모 1칸).
- **`codeAt(text, pos)`** = `bodyStart ≤ pos`이고 (`pos < bodyEnd` 또는 (닫히지 않은 블록이고 `pos === bodyEnd`))인 range, 없으면 `null`. 여는·닫는 펜스 줄 위는 `null`이다.
- **`langLabel(lang)`** = `lang`이 `''`면 `'text'`, 소문자로 바꾼 값이 `LANG_LABEL`(§5-7 — `Java` `JavaScript` `SQL` …)에 있으면 그 표기, 없으면 **받은 그대로**. 코드 블록 머리띠(§5-7)도 이 함수를 쓴다(표기 규칙이 한 곳).

**에디터의 "펜스 안"** — 걸친 줄(§6-2 "줄 범위" `[ls, le)`)에 대해 `r = codeAt(value, ls)`가 있고, (`le < r.bodyEnd` 또는 닫히지 않은 블록)이면 **안**. 곧 걸친 줄 전부가 **한** 블록의 코드 줄이다.
펜스 여는·닫는 줄 자체는 **밖**이다(짝·주석·Enter ①②③⑤가 발동하지 않는다 — v4.2와 같다). 언어가 필요한 규칙(주석 토글)은 `r.lang`을 쓴다 — 위쪽 펜스 줄을 다시 훑지 않는다.
`insideFence` · `codeLines` · `fenceLangAbove`의 판정 사본을 에디터에 남기지 않는다(`FENCE_LINE_RE` 삭제).

**합격 표 (판정 충돌 1 — 헤드리스 대조, 스크래치에서)** — 각 입력에서 `codeRanges`의 `[bodyStart, bodyEnd)` 줄들 = marked 12.0.2 `lexer`가 낸 **펜스** `code` 토큰(`codeBlockStyle`이
`'indented'`가 아닌 것)이 `<pre><code>`로 그리는 줄들이어야 한다. 탐침 줄 = 표의 "탐침" 줄이 코드 안인가.

| # | 입력 (줄은 `⏎`) | 탐침 | 기대 (= marked) | v4.2 `insideFence` |
|---|---|---|---|---|
| 1 | `~~~js⏎let a = (1);⏎~~~` | `let a` 줄 | **안**, `lang = 'js'` | 밖 ✗ |
| 2 | ````` ````md⏎```java⏎x⏎```⏎```` ````` (백틱 4개 펜스 안에 백틱 3개 예시) | ```` ```java ```` 줄과 `x` 줄 | **둘 다 안** — 블록은 하나(`lang = 'md'`), 안쪽 ``` 줄은 코드 글자 | 뒤집힘 ✗ |
| 3 | `   ```js⏎x⏎   ```` (3칸 들여쓴 펜스) | `x` | **안** | 밖 ✗ |
| 4 | `    ```js⏎    x` (4칸 — 들여쓴 코드) | `x` | **범위 없음**(펜스가 아니다. marked는 들여쓴 코드로 그리지만 대상 밖 — 위 규칙) | 밖 = 같음 |
| 5 | ```` ```java⏎a⏎```java⏎b⏎``` ```` (정보 문자열 붙은 줄은 닫는 펜스가 아니다) | 둘째 ```` ```java ```` 줄 · `b` | **둘 다 안** — 블록 하나, 마지막 ``` 가 닫는다 | 밖 ✗ |
| 6 | ```` ```x``` 뒤 문장⏎(괄호) ```` (줄 첫머리 인라인 코드) | `(괄호)` 줄 | **범위 없음**(백틱 펜스의 정보 문자열에 백틱 — 펜스가 아니다) | 안 ✗ |
| 7 | `- 항목⏎⏎  ```java⏎  int a;⏎  ```` (목록 항목 안 2칸 펜스) | `int a;` | **안**, `lang = 'java'` | 밖 ✗ |
| 8 | ```` ```js⏎x⏎``` ```` (평범한 블록) | `x` | **안** | 안 = 같음 |
| 9 | ```` ```Java⏎class A {}⏎``` ```` (§0-4 H1) | `class A {}` | **안**, `lang = 'Java'`(친 그대로) | 안 |
| 10 | ```` ```js⏎x ```` (닫는 펜스 없음 — 경계 문서) | `x` · 문서 끝 위치 | **안**, `close = bodyEnd = text.length` | 안 |
| 11 | 실제 글 사본 전체(§0-4 ①) | 모든 줄 | ```` ```Java ```` 블록 2개, 범위가 marked와 같다 | — |

frontend-dev가 벡터(입력 · 기대 범위)를 스크래치에서 헤드리스로 만들어 `codeRanges`를 대조하고, 결과를 frontend-dev-2에게 넘긴다(에디터 교체의 선행 조건).
벡터 파일은 프로젝트에 남기지 않는다(골든 벡터 M4-13 재개분과 합칠지는 U4 재개 결정에 따라 pm이 정한다).

#### 조합 확정 뒤 건너뛰기 (v4.3 — meeting-08 D4, §0-4 H4)

> 증상(로직 경로 실측, 실제 IME 순서는 미확인 → meeting-08 U7): 펜스 안 `print` 뒤 `(` → `(|)`, `"` → `("|")`, 한글 `안녕`을 조합하는 중에 `"` → 조합 가드가
> keydown을 돌려보내 브라우저가 `"`를 **넣는다** → `)`도 넣는다 → **`print("안녕")")`**. 짝을 넣는 것(조합 전)과 거두는 것(조합을 끝내는 그 키)이 갈렸다.

| 단계 | 규칙 |
|---|---|
| 표시 | `onBodyKeydown`의 조합 가드가 키를 돌려보낼 때, **그 키가 닫는 글자**(`)` `]` `}` `"` `'` `` ` ``)이고 **폼이 잠기지 않았고** 커서가 **펜스 안**이면 한 번짜리 표시를 남긴다: `{ ch: 그 글자, pos: 그때 커서 }`. `e.key`가 글자가 아니면(`Process` 등) 표시하지 않는다 — 보정할 글자를 모른다 |
| 보정 | **바로 다음 `input` 한 번**에서: 방금 들어온 글자(커서 바로 앞)가 표시의 `ch`이고, **커서 바로 뒤 글자도 같은 `ch`**면 → 뒤의 그 한 글자를 지운다(`replaceRange`). 결과 = 건너뛰기와 같다(`("안녕"|)`) |
| 버림 | 조건이 하나라도 어긋나면 아무것도 하지 않는다. 표시는 그 `input`에서(보정 여부와 무관하게) 지운다. `compositionend` 뒤 첫 `input`이 오지 않으면 다음 `keydown`에서 지운다 — 표시가 남아 엉뚱한 글자를 지우지 않게 |
| 되돌리기 | 보정은 **한 단계를 더** 만든다(브라우저가 넣은 글자 + 지운 글자). Ctrl+Z 한 번이면 보정 전(`("안녕"|")`)으로 돌아간다 — 받아들인다 |
| 이행 시점 | **U7 실기 확인 뒤**. 실제 Windows 한국어 IME에서 조합을 끝내는 문장부호 키가 `keyCode 229`로 오는지 사람이 1분 확인한다. 오지 않으면(가드가 돌려보내지 않으면) 이 보정은 필요 없고 v4.2 건너뛰기가 이미 맞다 — 그때는 이 소절을 "해당 없음"으로 닫는다 |

#### 코드 블록 안의 짝 (v4.2 — 사용자 원문 *"코드 블록 부분 괄호 쓰면 IDE 처럼 괄호 닫히는거 까지 같이 보여줘"*, `onPairKey()` · `onPairBackspace()`)

"펜스 안" = 걸친 줄 **전부**가 한 코드 블록 안의 코드 줄 — ~~위쪽 펜스 줄 수가 홀수(`insideFence`, `//` 규칙과 같은 함수)이고, 걸친 줄 가운데
```` ``` ````로 시작하는 줄이 **하나도 없다**~~ **v4.3: 위 "코드 블록 안 = `codeRanges`"의 "에디터의 펜스 안" 한 문장**(펜스를 여는·닫는 줄 자체에서는 짝·주석 규칙이 발동하지 않는다 — 그대로). 보조키: Ctrl·Alt·Cmd 없음(Shift는 보지 않는다 —
US 자판에서 `(` `{` `"`는 Shift로 나오는 글자이고 `e.key`가 이미 그 글자다).

| 동작 | 키 | 조건 | 결과 |
|---|---|---|---|
| 감싸기 | 여는 글자 `(` `[` `{` `"` `'` `` ` `` | 선택이 있다 | 선택을 `여는 + 선택 + 닫는`으로. 선택은 안쪽 글 그대로 유지. 닫는 글자(`)` 등)를 선택 위에 치면 기본 동작(선택을 그 글자로 바꾼다) |
| 건너뛰기 | 닫는 글자 `)` `]` `}` `"` `'` `` ` `` | 선택 없음, **커서 바로 뒤 글자가 친 글자와 같다** | 글자를 넣지 않고 커서만 한 칸 오른쪽. **자동 닫기보다 먼저 검사한다** — 따옴표는 여는 글자이자 닫는 글자라 `"|"`에서 `"`는 넘어가야 한다. `}` `)` `]`는 Enter 규칙 ⑤보다도 먼저다 |
| 자동 닫기 | 여는 글자 | 선택 없음, **다음 글자**가 줄 끝·문서 끝·공백 또는 `)` `]` `}` `;` `,` `.` 중 하나 | 짝을 함께 넣고 커서는 사이(`(|)`). 글자 앞에서 여는 괄호를 치면 대개 그 글자를 감쌀 생각이라 짝을 넣지 않는다 |
| 따옴표 예외 | `"` `'` `` ` `` | 위 조건에 더해 **앞 글자가 단어 글자**(유니코드 문자·숫자·`_`)이거나 **같은 따옴표**면 짝을 넣지 않는다 | `don't` · 이미 연 따옴표 뒤. 백틱 셋으로 펜스를 닫을 때: 첫 백틱 = 빈 쌍(커서 사이) → 둘째 = 건너뛰기(백틱 둘, 커서 뒤) → 셋째 = 앞 글자가 백틱이라 짝 없음(백틱 셋) — 그대로 닫힌다 |
| 쌍 지우기 | Backspace(보조키 없음, Shift도 없음) | 선택 없음, 커서가 빈 쌍 `(|)` `[|]` `{|}` `"|"` `'|'` `` `|` `` 사이 | 두 글자를 함께 지운다 |

- **알려진 차이(받아들인다)** — IDE는 "자기가 넣은 짝"만 건너뛴다. textarea는 그 상태를 추적할 수 없어 **바로 뒤가 같은 글자면 언제나 건너뛴다**(v3.9가
  자동 닫기를 기각한 근거 "type-over를 추적할 수 없다"를 이 규칙으로 받아들였다). 같은 닫는 괄호를 연달아 두 개 쓰고 싶으면 두 번째는 커서를 옮긴 뒤 친다 —
  코드 안에서 드문 모양이고, 결과가 화면에 그대로 보여 틀린 줄을 모른 채 넘어가지 않는다.
- **펜스 밖에서는 v3.9 그대로 하지 않는다** — 마크다운의 `[텍스트](url)`·`(괄호 속 문장)`과 충돌한다.

#### 주석 토글 (v4.2 — Ctrl+/, 펜스 안, `toggleComment()`)

접두는 ~~커서 위쪽에서 마지막 펜스 줄(= 지금 블록을 연 줄)의 정보 문자열 첫 낱말~~ **v4.3: 그 블록의 `codeRanges` `lang`**을 **소문자로 바꿔서** 아래 표에서 찾는다(```` ```Java ````도 `java`로 찾는다 — D18).

| 방식 | 표시 | 언어 (v4.3 추가는 **굵게**) |
|---|---|---|
| 줄 접두 | `//` | `java` `js` `javascript` `ts` `typescript` `c` `cpp` `cs` `csharp` `kotlin` `kt` `go` `rust` `swift` `scss` `dart` `php` **`jsx` `tsx` `groovy` `gradle` `scala` `less`** — ~~그리고 언어 없음 · 표에 없는 언어(기본)~~ **v4.3: 기본값이 아니다** |
| 줄 접두 | `#` | `python` `py` `bash` `sh` `shell` `yaml` `yml` `ruby` `rb` `r` `toml` `dockerfile` **`powershell` `ps1` `ps` `perl` `pl` `makefile` `make` `properties` `conf` `nginx` `gitignore` `zsh`** |
| 줄 접두 | `--` | `sql` **`lua` `haskell` `hs`** |
| 줄 접두 | **`;`** | **`ini`** |
| 줄 접두 | **`REM`** | **`bat` `cmd` `batch` `dos`** — `::`가 아닌 이유: 괄호 블록(`if (…) ( … )`) 안의 `::`는 cmd가 잘못 읽는다. `REM`은 어디서나 주석이다 |
| 줄 접두 | **`%`** | **`tex` `latex` `matlab`** |
| 감싸기 | `<!--` … `-->` | `html` `xml` `md` `markdown` **`svg` `vue`** |
| 감싸기 | `/*` … `*/` | `css` |
| **무동작** (v4.3) | — | **표에 없는 언어 · 언어 없는 블록 · 주석 문법이 없는 언어(`json` `text` `plaintext` `txt` `diff` `csv`)** |

- **무동작(v4.3 — meeting-08 D20)**: 글을 바꾸지 않고 키는 먹는다(`preventDefault` — 브라우저 기본 Ctrl+/로 새지 않게). 토스트(기본 종류, 3.2초) 한 줄:
  언어가 있으면 **`<lang>은 주석 표시를 몰라 넣지 않았어요`**(`<lang>`은 친 그대로), 언어가 없으면 **`언어가 없는 코드 블록이라 주석 표시를 정할 수 없어요`**.
  v4.2의 "모르면 `//`"는 PowerShell 블록에 `// Get-Item`을 조용히 썼다 — 틀린 주석은 코드를 깨고, 눈에 잘 띄지 않는다. 안 한 것은 토스트가 말한다.
  `<lang>은`의 조사는 받침과 무관하게 `은`으로 고정한다(글자가 영문이라 받침 판정이 흔들린다 — 짧은 안내에서 한 번 어색한 편이 코드를 늘리는 것보다 낫다).
- 표를 늘리는 곳은 이 계약이 먼저다. 사용자가 실제로 다루는 언어(이 저장소의 `start.ps1`·`start.bat` 포함)를 기준으로 고른다.

- **줄 접두**: 비지 않은 줄들의 **최소 들여쓰기** 자리에 `표시 + 공백 하나`를 넣는다(줄들의 들여쓰기 모양이 그대로 남는다). 비지 않은 줄이 **전부** 이미 그 표시로
  시작하면(들여쓰기 뒤) 푼다 — 표시와 그 뒤 공백 하나를 지운다. 빈 줄은 건드리지 않는다.
- **감싸기**: 첫 비지 않은 줄의 들여쓰기 뒤에 `여는 표시 + 공백`, 마지막 비지 않은 줄의 끝(끝 공백 앞)에 `공백 + 닫는 표시`. 이미 그렇게 감싸여 있으면 푼다.
- **빈 줄 하나**에서 누르면 표시만 넣고 커서를 그 안에 둔다(`// |` / `<!-- | -->`). 빈 줄 여럿이면 아무것도 하지 않는다(키는 먹는다).
- 커서·선택은 편집을 따라 옮겨진다(지워진 글자 안에 있던 위치는 그 편집 바로 뒤로 — v4.3 "편집 뒤 선택"과 같은 규칙). 언어 판정은 `hasOwnProperty`로 한다 — `constructor` 같은 이름이 기본 객체의 것을 집지 않게.

#### 코드 블록 언어 기억 (v4.2 — localStorage `blogCodeLang`)

| 항목 | 규칙 |
|---|---|
| 값 | ~~`[a-z0-9+#-]{1,20}`(소문자만)~~ **v4.3(D18): `[A-Za-z0-9+#-]{1,20}`, 친 그대로 저장한다**(`Java`는 `Java`로 — 툴바가 다음에 ```` ```Java ````로 연다. §0-4 H1). 없거나 규칙에 안 맞으면(손으로 고친 저장소 등) **기본 `java`**(§5-7-1 "IntelliJ — 자바 기준"과 같은 기본). "같은 값은 다시 쓰지 않는다"의 같음은 **글자 그대로**(대소문자가 다르면 새 값 — 사용자가 표기를 바꾼 것이다) |
| 쓰는 곳 셋 | ① `//언어` + Enter/Space로 펜스를 만들 때(언어 없는 `//`는 기억을 바꾸지 않는다) ② 커서가 펜스 **여는** 줄에 있고 그 줄 전체가 ```` ```언어 ````(뒤 공백만 허용)일 때 — 툴바가 선택해 둔 언어를 덮어쓴 경우 등. 정보 문자열이 더 붙은 줄(```` ```js title="a" ````)·닫는 줄은 배우지 않는다. ~~대문자(```` ```Java ````)는 배우지 않는다~~ **v4.3: 대문자도 배운다.** "여는 줄"의 판정은 `codeRanges`의 `open` 줄이다(v4.3 — `~~~`로 연 줄은 모양이 ```` ``` ````가 아니라 배우지 않는다: 툴바는 ```` ``` ````로 연다) ③ (읽기만) 툴바 `코드 블록` |
| ②의 성질 | `input` 때마다 그 줄의 **지금 모양**을 배운다 — 언어를 지우는 도중의 모양(```` ```p ````)도 마지막 값이 될 수 있다. 다음 툴바가 그 값으로 열어도 언어 글자가 선택돼 있어 한 번에 덮어쓴다(받아들인다). 같은 값은 다시 쓰지 않는다 |
| 키의 자리 | ~~`editor.js` 안 상수(`CODE_LANG_KEY`)~~ **v4.3: `CFG.storageKeys.codeLang`(`'blogCodeLang'` — 값 유지, 옛 기억이 그대로 읽힌다)** — §3-2 "저장 키 표". v4.2의 이행 판단("파일 하나뿐이라 상수로")은 키 목록을 한 곳에 모으는 규칙으로 대체됐다. `editor.js`는 `config.js`의 값이 없을 때만 문자열로 폴백한다 |
| 막혔을 때 | localStorage 예외(시크릿 모드 등)는 삼키고 기본값으로 계속 간다 |

#### 알리는 자리 (v4.2 — write.html 문구)

| 자리 | v4.1 (v4.2 이전 write.html) | v4.2 (현재 write.html) |
|---|---|---|
| `#bodyHint` | `Tab은 들여쓰기입니다. 포커스를 다음 항목으로 옮기려면 Esc를 누른 뒤 Tab을 누르세요. Ctrl+B 굵게, Ctrl+I 기울임, Ctrl+S 저장. 줄 첫머리에 //를 치고 Enter 또는 Space를 누르면 코드 블록이 됩니다. 코드 블록 안에서 Enter는 들여쓰기를 이어 가고, 목록 줄에서 Enter는 다음 항목을 만듭니다. 그냥 줄만 바꾸려면 Shift+Enter.` | **`Tab은 들여쓰기, Shift+Tab은 내어쓰기입니다. 포커스를 다음 항목으로 옮기려면 Esc를 누른 뒤 Tab을 누르세요. Ctrl+B 굵게, Ctrl+I 기울임, Ctrl+S 저장, Ctrl+D 줄 복제, Alt+Shift+위쪽 또는 아래쪽 화살표 줄 이동. 줄 첫머리에 //를 치고 Enter 또는 Space를 누르면 코드 블록이 됩니다. 코드 블록 안에서는 여는 괄호와 따옴표를 치면 닫는 짝이 함께 들어가고, Ctrl+/는 주석을 넣거나 뺍니다. 코드 블록 안에서 Enter는 들여쓰기를 이어 가고, 목록 줄에서 Enter는 다음 항목을 만듭니다. 그냥 줄만 바꾸려면 Shift+Enter.`** |
| 툴바 `[data-md="codeblock"]` | `aria-label="코드 블록 (// 로도)"`, `title` 없음, 문구 `코드 블록` | **그대로.** 단축키가 없으니 `title`도 없다(§6 `title` 규칙 — 남는 `title`은 여전히 셋). 기억한 언어는 넣은 뒤 선택된 언어 글자가 말한다 — 이름에 언어를 넣으면 JS가 이름을 갈아 끼워야 하고, 스크린리더는 버튼을 지날 때마다 그 말을 듣는다 |
| `[data-md="bold"]` `[data-md="italic"]` | `title="굵게 (Ctrl+B)" aria-keyshortcuts="Control+B"` / `title="기울임 (Ctrl+I)" aria-keyshortcuts="Control+I"` | **그대로.** `title`과 `aria-keyshortcuts`는 짝이라 같은 키를 말해야 한다(§6) — Cmd는 화면 문구에 적지 않는 규칙(위)과 같다 |
| `#fBody` | `aria-keyshortcuts` 없음 | **그대로 — 붙이지 않는다.** `aria-keyshortcuts`는 "그 요소를 누르거나 포커스하는 키"를 말하는 속성이다. 본문 안의 편집 명령(Ctrl+D 등)은 대상이 아니다 — 그 안내는 `aria-describedby="bodyHint"`가 맡는다 |
| placeholder | `마크다운으로 씁니다. 줄 첫머리 // 는 코드 블록, Tab은 들여쓰기(빠져나가려면 Esc 뒤 Tab).` | **그대로**(v4.1 "한 문장" 규칙 — 쓰기 시작하면 사라지는 자리에 단축키 나열을 담지 않는다) |
| 상태줄 `#editorStatus` | Tab 안내 둘 | **그대로** — v4.2는 새 상태줄 문장을 만들지 않는다(§6-1 "상태줄 문구" 목록 변화 없음). 선택 없는 Shift+Tab도 첫 Tab 안내(`teachTab`)를 띄운다 |

`#bodyHint`는 화면 어휘다(원칙 7) — 화살표 글자(`↑` `↓`)를 쓰지 않고 `위쪽 또는 아래쪽 화살표`로 적는다. 문장 안의 나열은 쉼표. 마지막 문장(Enter)은 v3.9 문장 **글자 그대로**다(아래 "Shift+Enter가 탈출구다").
...
> +1단 등)은 넣지 않는다 — dev-2 확정 지식. **펜스 밖에서는 자동 닫기 괄호를 넣지 않는다** — 마크다운의 `[텍스트](url)`과 충돌한다.
> ~~type-over 상태를 textarea에서 추적할 수 없다~~ → **v4.2: 펜스 안에서는 짝을 넣는다**(위 "코드 블록 안의 짝"). type-over를 추적할 수 없다는 제약은
> 그 절의 "알려진 차이"(바로 뒤가 같은 글자면 언제나 건너뛴다)로 받아들였다.

공통 전제 — 발동 조건은 `e.key === 'Enter'`, 보조키 없음(`shift/ctrl/alt/meta` 전부 false), 선택 없음(`selectionStart === selectionEnd`),
조합 중 아님. 하나라도 어긋나면 기본 동작이다. "현재 줄" = ~~`value.lastIndexOf('\n', pos - 1) + 1`~~ **v4.3: `lineStartAt(value, pos)`**(위 "줄 시작" — 공식이 아니라 동작 문장)부터 `pos`까지(`before`)와 `pos`부터
다음 `\n`까지(`after`). `indent` = `before`의 앞 공백(`/^[ \t]*/`). "펜스 안" = ~~`insideFence(value.slice(0, lineStart))`(커서 위쪽의 줄 첫머리 ```` ``` ```` 줄 수가 홀수)~~
**v4.3: 커서 줄이 "코드 블록 안 = `codeRanges`"의 코드 줄**(§6-2 `//` 규칙과 같은 판정). ④(목록 이어쓰기)는 "펜스 밖"이 조건이라 코드 안의 `- `·`> `로 시작하는 줄에 더는 붙지 않는다(v4.2 결함).
...
- **위치와 순서** — `onBodyKeydown`에서 Tab 처리 **뒤**, `tabEscape` 해제 줄 **뒤**, `Ctrl` 검사 **앞**. Enter는 ①②③(펜스 안) 아니면 ④(펜스 밖)
  ...
  **v4.2 — `onBodyKeydown()` 전체 순서**: 조합 중·잠김이면 끝(**v4.3**: 조합 중이면 끝나기 전에 "조합 확정 뒤 건너뛰기"의 표시만 남긴다 — `preventDefault` 없음) → Esc(탈출 대기) → Tab/Shift+Tab(Ctrl·Alt·Cmd가 붙으면 손대지 않음) → 탈출 대기 해제 →
  Enter(①~④) → Alt+Shift+↑/↓(줄 이동) → **보조키(Ctrl·Alt·Cmd) 없는 글자 키**: Backspace(쌍 지우기) · 짝(건너뛰기 → 감싸기/자동 닫기) · ⑤ →
  **Ctrl 조합**(보조키 규칙): B · I · D · /. 처리하지 않은 키는 전부 브라우저 기본이다.
...
- **하지 않는 것** — **펜스 밖의** 자동 닫기 괄호(위 — 펜스 안은 v4.2 "코드 블록 안의 짝"), 언어별 규칙(`:` `=>` `then`), 렌더된 코드 블록의 줄번호(부품·계약 신설이 필요하고 사용자 요청이 없다 —
...
- 언어 붙이기: `//` 뒤에 ~~`[a-z0-9+#-]{1,20}`~~ **`[A-Za-z0-9+#-]{1,20}`(v4.3 — 친 그대로 쓴다)**가 이어지면 그것이 언어다. **Enter 또는 Space**가 확정 신호다 — `//` 둘만 치고 Enter/Space면
...
- 블록 안 판정: ~~커서 앞 텍스트에서 줄 첫머리 ```` ``` ```` 줄을 세어 홀수면 "블록 안". `~~~` 펜스는 세지 않는다(이 에디터의 툴바가 만들지 않는 문법).~~
  **v4.3: `Blog.markdown.codeAt(value, 줄 시작)`**(위 "코드 블록 안 = `codeRanges`"). "툴바가 만들지 않는 문법은 세지 않는다"는 **폐기** — 판정의 기준은 생산자(툴바)가 아니라
  독자가 보는 결과(렌더러)다. 붙여 넣은 README의 `~~~`·목록 안 펜스·백틱 4개 펜스도 코드다(§0-4 ③).
...
.title-type .title-typed .title-rest                    (v4.4 — 제목 타자. 첫 방문에만 app.js가 h1.page-title 안에 만들고 끝나면 지운다, §3-5. 커서는 .title-rest::before)
...
**v4.4: `.intro` `.intro-bulb` `.intro-skip` · `#intro` `#introSkip` · 인트로 동안의 `inert` · 인트로 Esc · `#intro[hidden]` · 키프레임 `intro-out` `intro-bulb-rise` `intro-bulb-ting` `intro-bulb-exit` `intro-bulb-on`(§3-5 — 전구 인트로 폐기)**
...
**v3.8 — 목록 컨테이너의 속성 `#postList[data-ready]`.** 클래스가 아니라 속성이다(첫 방문의 `data-intro`와 같은 태도 —
...
**v4.0 — 속성 하나가 더 있다.** **`#postList[data-cats]`** — 글이 있는 분류의 수(블로그 전체 기준, 필터와 무관). `app.js`가 글을 받은 뒤
`renderList()` 전에 한 번 적는다. CSS는 `.post-list[data-cats="1"] .entry-cat { display: none }` 한 줄(§4-4). 속성이 없으면 라벨은 보인다.
(v4.0의 둘째 — 인트로 동안의 `inert` — 는 v4.4에 막과 함께 폐기. `inert`는 모달(§7)의 배경에만 남는다.)
...
**v4.4 — 첫 방문 연출(§3-5)의 상태는 클래스가 아니다.** `html[data-intro="done"]` 하나 — `theme-init.js`가 첫 페인트 전에,
`sessionStorage['blogIntro']`가 있을 때 붙이고 **그 뒤로는 누구도 붙이거나 떼지 않는다**(`data-side`와 다르다 — 페이지가 사는 동안 한 값).
타자의 진행에도 상태 클래스가 없다 — **DOM의 내용이 곧 상태**다: `.title-rest:empty` = 다 썼다 → 깜빡임(CSS가 읽는다). `is-*`를 붙이지 않는다.
...
| **제목 타자 (v4.4 · 박자 v4.5 — §3-5)** | 첫 글자 = 제목의 `title-wait`가 **끝나는 순간**(사건으로 맞춘다 — 600을 JS에 복사하지 않는다). 글자 간격 ~~120ms~~ **160ms**. 첫 글자부터 끝 글자까지 ~~1200ms~~ **1600ms 상한**(넘는 긴 제목은 간격을 줄인다). 끝 글자 뒤 CSS 깜빡임이 끝나면(`.title-rest`의 `animationend`) 평문 — 이벤트가 새도 **첫 글자 뒤 5000ms**면 평문(안전망 — v4.5 그대로) | `title-wait` = `--dur-intro`(600ms). 깜빡임 = `--dur-caret-blink`(~~1000~~ **1100ms**) × 3 = **3300ms**. 6글자면 첫 글자 600 · 끝 글자 1400 · 평문 4700 / 상한이면 끝 글자 2200 · 평문 5500(첫 글자부터 4900 — 안전망보다 100ms 먼저. 그 100ms가 늦어져도 깜빡임 마지막 반 박자 550ms는 꺼짐이라 안전망이 먼저 와도 보이는 차이가 없다) |

이 숫자를 JS에서 줄이면 요소가 사라지는 중간에 잘린다. 늘릴 때는 상관없다. **단 제목 타자는 예외다** — 간격·상한을 늘리면 5초(WCAG 2.2.2)를 넘는다. 늘리려면 이 표와 §3-5("5초 예산"의 식)를 먼저 고친다.
제목 타자의 시작은 JS가 시간을 세지 않는다 — CSS 대기가 끝났다고 알려 줄 때 첫 글자를 쓴다(§3-5). 안전망 타이머만 CSS보다 길다.
...
**v3.6 → v4.4: index.html의 `.site-header` `.site-main` `.site-footer`가 첫 방문 로드에 0에서 시작한다(§3-5 ①) — 규칙 위반이 아니다.**
이 규칙이 막는 것은 "JS가 성공해야 1이 되는 정적 요소"다. 올라오기의 0→1은 CSS `@keyframes intro-arrive`가
600ms에 스스로 끝내고, JS 실패·`:has()` 미지원·reduced-motion 어느 경우에도 영구 불가시가 생기지 않는다
(각각 CSS가 끝냄 / 선택자 불일치로 애니메이션 자체가 없음 / `animation: none`). 판별 기준은 "누가 1로 만드는가"다.
**v4.4: 같은 이유로 `h1.page-title`의 글자가 첫 방문 로드에 600ms 투명하다(`title-wait`, §3-5 ②) — 위반이 아니다.** 보이게 하는 것은 CSS 애니메이션의 끝이지 JS가 아니다.
JS가 실패하면 정적 제목이 600ms에 완성된 채 보인다. 타자 DOM의 `.title-rest`(투명 글자)는 JS가 만든 요소라 규칙의 대상 밖이다.
...
```
첫 줄 문구는 **요구사항 #16 그대로 유지**한다. 세 페이지 동일(index · post · write — 세 줄 모두, 이 순서).

**v4.2: 둘째 `.footer-note` — 사용자 추가 문구(요구사항 #16과 별개).**
사용자 원문: *"푸터 문구에 추가하는데 문장은 "본 게시글 내용은 직접 작성하여 정리했습니다." 이걸 문법적 오류와 설명, 가독성, 현재 구조에 맞게
푸터 메인화면에 추가해"*. 선택지 답변으로 확정한 것: 문장 = `게시글은 모두 직접 작성하고 정리하였습니다`, 범위 = 세 페이지 모두.

| 원문에서 | 확정 문장에서 | 이유 |
|---|---|---|
| `본 게시글` | `게시글은 모두` | 푸터는 목록 화면(index)에도 있다 — "본"(이 글)은 글 하나를 가리켜 목록에서 틀린 말이 된다. "모두"가 범위를 말한다 |
| `게시글 내용은` | `게시글은` | "게시글"이 이미 내용이다 — 겹침 제거 |
| `작성하여 정리했습니다` | `작성하고 정리하였습니다` | 두 동작을 대등하게 잇는다 · 어미를 첫 줄(`제작하였습니다`)과 같은 "하였습니다"로 통일 |
| 끝의 `.` | 없음 | 첫 줄에 마침표가 없다 — 푸터 두 줄의 문장 부호를 맞춘다 |

- 요구사항 #16의 원문이 **아니다**. 첫 줄의 "손대지 않는다" 규칙은 첫 줄에만 걸린다 — 이 줄을 바꾸려면 사용자 확인을 다시 받는다.
- 클래스를 새로 만들지 않는다 — 두 문장은 같은 층(읽으라고 쓴 문장, `--c-text-dim`)이고 생김새가 같아야 한다. `:nth-child`로 둘을 다르게 꾸미지 않는다.
- 작업 트리에 먼저 들어가 있던 `게시글 내용은 직접 손수 작성하였습니다`는 미확정안이었다 — 이 문장으로 **교체**했다(§12-18 #158·#159 ✔).

**간격(v4.2, `components.css` §12)** — 두 문장 모두 마침표가 없어서, 360에서 첫 줄이 두 줄로 꺾이면 "꺾인 줄"과 "다음 문장"이
같은 행간으로 이어져 한 문단처럼 읽혔다(헤드리스 360 실측). 그래서 층을 간격으로 나눈다:

| 사이 | 값 | 뜻 |
|---|---|---|
| 한 문장 안(꺾인 줄) | 0 — 행간만 | 같은 문장 |
| `.footer-note + .footer-note` | `--sp-2`(8) | 다른 문장, 같은 층 |
| `.footer-note` → `.footer-meta` | `--sp-3`(12, v4.1까지 `--sp-2`) | 다른 층(문장 묶음 / 연도) |
...
| 대상 | 무엇이 | 시간 |
|---|---|---|
| **`:root` — 테마 크로스페이드 (v3.4)** | **색 토큰 40개**(`--c-*` 20 · `--code-*` 3 · `--hl-*` 17, `@property`로 등록 — v3.8 `--c-bg-2` 삭제 · v4.1 `--c-border-strong` · v4.4 `--c-glow` 삭제) | `--dur-theme`(1200ms) + `--ease-fade` |
...
| **`.site-header` `.site-main` `.site-footer` — 페이지 올라오기 (v4.4, `@keyframes intro-arrive`, index.html·첫 방문만)** | `opacity` 0→1 + `translate` **12px**→0 | `--dur-intro`(600) + `--ease`, **delay 0**, `backwards` |
| **`.page-title` — 글자 대기 (v4.4, `@keyframes title-wait`, index.html·첫 방문만)** | `color` = `transparent`(시작·끝 같은 값 — 보간 없음, 끝나면 원래 색) | `--dur-intro`(600) + `linear`, delay 0, fill 없음 |
| **`.title-rest:empty::before` — 커서 깜빡임 (v4.4, `@keyframes title-caret-blink`)** | `opacity` 1 → 0(반 박자씩, 사이 보간 없음) | `--dur-caret-blink`(1100 — v4.5) + `linear`, **3회**, `forwards`(끝은 꺼짐) |
...
- **전환은 요소가 아니라 토큰에 걸린다.** `tokens.css`가 색 토큰 40개(v4.4)를 `@property … syntax: "<color>"`로
...
| **첫 방문 연출의 골격 — `@keyframes intro-arrive`(골격 셋) · `@keyframes title-wait`(`.page-title`) · 둘을 감싼 `(prefers-reduced-motion: no-preference)` 블록** (v4.4) | `layout.css` §10 — 골격 셋을 움직이고 `.page-title`(§3)의 글자를 기다리게 하는 것은 골격의 일이다. `.page-title`의 규칙이 한 파일에 모인다 |
| **제목 타자 부품 — `.title-type` `.title-typed` `.title-rest` + `::before`(커서) · `@keyframes title-caret-blink`** (v4.4) | `components.css` §16 — v3.6~v4.3의 전구 자리. 커서와 그 깜빡임은 부품의 모양·모션이다 |
...
### 12-12. v3.6 이행 — 인트로 전등 (frontend-dev #70·#72 · frontend-dev-2 #71·#73-75) — **v4.4 폐기(§3-5 · §12-20). 이 표는 이력이다**
...
### 12-18. v4.2 이행 — 코드 블록 편의 (#150-156) · 푸터 둘째 줄 (#157-159)

> **사후 등재다.** #150-155는 frontend-dev-2가 `editor.js`에 먼저 구현했고(머리 주석 v4.2 · 미커밋), 이 개정이 그 동작을 §6-2의 명세로 올렸다 — ✔는
> "코드가 계약과 같다"는 뜻이다(web-designer가 `git diff js/editor.js`를 함수 단위로 대조). #150-156 담당은 전부 frontend-dev-2(§1-1), CSS 변화 없음.
> **#156 이행(2026-09-23, frontend-dev-2)**: `write.html` `#bodyHint` 문장이 §6-2 "알리는 자리(v4.2)" v4.2 칸과 글자 일치(web-designer 재대조 — 바이트 비교 일치) ·
> 헤드리스 셀프체크 32항목 통과. **실키보드·IME 입력은 미확인.**
> **#157-159는 v4.2 추가 확정(§10 푸터 둘째 줄)이다** — 사용자 원문과 문장 확정 경위는 §10. #158 · #159 ✔(2026-09-23, web-designer가 세 HTML의 둘째 `.footer-note`를 글자 대조).

| # | 담당 | 파일 · 위치 | 할 일 | 근거 | 상태 |
|---|---|---|---|---|---|
| 150 | frontend-dev-2 | `editor.js` `onPairKey()` `onPairBackspace()` `codeLines()` `codeAt()` · `onBodyKeydown()` 글자 키 갈래 | 펜스 안 감싸기 · 건너뛰기(⑤보다 먼저) · 자동 닫기(다음 글자 조건) · 따옴표 예외(앞 글자 단어 글자·같은 따옴표) · 쌍 지우기. 펜스 여는/닫는 줄 제외. `replaceRange` 한 번 | §6-2 "코드 블록 안의 짝" | ✔ |
| 151 | frontend-dev-2 | `editor.js` `toggleComment()` `commentStyle()` `fenceLangAbove()` `lineCommentEdits()` `wrapCommentEdits()` `applyLineEdits()` | Ctrl+/ 펜스 안만(밖이면 `preventDefault` 없음) · 언어별 접두 표 · 최소 들여쓰기 · 전부 주석이면 풀기 · 감싸기 · 빈 줄 하나 = 표시만 · 커서·선택 옮김 | §6-2 "주석 토글" | ✔ |
| 152 | frontend-dev-2 | `editor.js` `lineSpan()` `duplicateLines()` `moveLines()` | 줄 범위(다음 줄 0열 제외) · Ctrl+D 복제(선택이 복제본으로) · Alt+Shift+↑/↓ 이동(끝에서는 키만 먹음) | §6-2 표 · "줄 범위" | ✔ |
| 153 | frontend-dev-2 | `editor.js` `onBodyKeydown()` Tab 갈래 | **결함 수정**: 선택 없는 Shift+Tab이 v4.1까지 여러 줄 갈래를 타서 **줄 전체가 선택된 채** 끝났다 — 이어서 글자를 치면 그 줄이 통째로 사라졌다(v3.8 "Tab의 한 줄 선택 소결함"과 같은 꼴의 덫). 표의 "현재 줄 첫머리에서 공백 최대 2칸 제거"는 v3.8부터 그대로였고 구현이 어긋났던 것이다. v4.2 표에 **"커서는 같은 글자 앞"** 을 명시. 함께: 선택 있는 Tab의 줄 범위 = `lineSpan()`, 바뀐 글자가 없으면 `replaceRange` 생략, Ctrl·Alt·Cmd+Tab은 가로채지 않음 | §6-2 Tab 행 | ✔ |
| 154 | frontend-dev-2 | `editor.js` `insertCodeBlock()` `codeLang()` `rememberCodeLang()` `learnFenceLang()` · `maybeFence()` 끝 · `onBodyInput()` | 툴바 `코드 블록` = 기억한 언어(기본 `java`) + 언어 글자 선택 + 줄 중간이면 앞 줄바꿈, `// 코드` 자리표시 폐기. 기억은 `//언어`·펜스 여는 줄에서. 키 `blogCodeLang`은 `editor.js` 상수(이행 판단 등재) | §6-2 "코드 블록 언어 기억" | ✔ |
| 155 | frontend-dev-2 | `editor.js` `keyIs()` · `onBodyKeydown()` Ctrl 갈래 | 보조키 규칙 — Ctrl·Cmd 정확히 하나 + Alt·Shift 없음(B · I · D · /). 한글 자판은 `e.code`로 한 번 더 | §6-2 "보조키 규칙" 행 | ✔ |
| 156 | frontend-dev-2 | `write.html` `#bodyHint` | 문장을 §6-2 "알리는 자리(v4.2)" 표의 v4.2 칸 **글자 그대로**로 교체. 나머지 속성·툴바·placeholder는 그대로. 원하면 바로 위 HTML 주석에 `v4.2: 코드 블록 편의(짝·Ctrl+/·Ctrl+D·Alt+Shift+화살표)를 #bodyHint가 말한다` 한 줄 | §6-2 "알리는 자리" | ✔ |
| 157 | web-designer | `docs/contract.md` §3 · §10 · `css/components.css` §12 푸터 | 푸터 둘째 `.footer-note` 등재(사용자 추가 문구, 요구사항 #16과 별개) · 간격 `.footer-note + .footer-note` = `--sp-2`, `.footer-meta` 위 `--sp-2` → `--sp-3`. 클래스 신설 없음 | §10 | ✔ |
| 158 | frontend-dev | `index.html` · `post.html` `<footer class="site-footer">` 둘째 `<p>` | `<p class="footer-note">게시글 내용은 직접 손수 작성하였습니다</p>` → `<p class="footer-note">게시글은 모두 직접 작성하고 정리하였습니다</p>`. 첫 줄·`.footer-meta`·순서는 그대로 | §10 | ✔ |
| 159 | frontend-dev-2 | `write.html` `<footer class="site-footer">` 둘째 `<p>` | #158과 같은 교체(글자 그대로) | §10 | ✔ |

**확인 방법(담당자 셀프체크)** — 펜스 ```` ```java ```` 안에서: #150 `foo(` → `foo(|)`, 이어서 `)` → `foo()|`(글자 하나) / 단어 선택 후 `"` → `"단어"`(선택 유지) /
`a|b`에서 `(` → 짝 없음 / `don` 뒤 `'` → 짝 없음 / `(|)`에서 Backspace → 둘 다 사라짐 / 빈 줄에서 `` ` `` 셋 → ```` ``` ```` 하나로 블록이 닫힌다 / 펜스 **밖**에서 `(` → 짝 없음.
#151 두 줄 선택 Ctrl+/ → 둘 다 `// `, 다시 → 풀림 / ```` ```python ```` 안 → `# ` / ```` ```html ```` 안 → `<!-- … -->` / 펜스 밖 Ctrl+/ → 아무 일 없음. #152 Ctrl+D → 아래에 복제, 커서가 복제본에 /
Alt+Shift+↑ → 줄이 위로, 첫 줄에서는 그대로. #153 `    foo|` 에서 Shift+Tab → `  foo|`(선택 없음) → 한 글자 입력해도 줄이 남는다. #154 `//python` + Enter 뒤 툴바 `코드 블록` →
```` ```python ````에 `python`이 선택돼 있다 / 줄 중간에서 누르면 펜스가 새 줄에서 시작. #155 본문에서 Ctrl+Shift+B → 굵게가 되지 않는다. 전 항목 Ctrl+Z 한 번에 한 동작.
#156: 스크린리더(또는 DevTools 접근성 트리)에서 본문 칸 설명이 새 문장.
#158·#159: 세 페이지에서 `document.querySelectorAll('.site-footer .footer-note')`가 2개, 둘째의 `textContent`가 `게시글은 모두 직접 작성하고 정리하였습니다`(마침표 없음) /
`grep -n "손수" index.html post.html write.html` 결과 없음 / 360 폭에서 첫 줄이 꺾여도 두 문장 사이에 틈(8px)이 보인다.

### 12-19. v4.3 이행 — 덮어쓰기 차단 · 에디터 판정 = 렌더러 · 습관 입력 (#160-179)

> **계약 먼저, 그다음 코드.** 이 표에 없는 동작·문구·저장 키를 만들지 않는다(v4.2의 사후 등재를 되풀이하지 않는다 — meeting-08 D6). 이행 중 원안과 다르게 판단할 일이 생기면
> 코드를 쓰기 **전에** web-designer에게 개정을 요청한다. **✔ 조건은 §0-4다** — 보고에 입력으로 쓴 습관 행(H1~H7)과 경계 문서를 적는다.
> 선행: **#162 → #169 · #170 · #173**(에디터 교체는 `codeRanges` 합격 뒤) · **api.md v1.2(pm) → #177**(서버 필드명) · **#165 → #175** · **U7 → #179**.
> 한 파일을 두 사람이 만지지 않는다(§1-1). 줄번호보다 함수 이름으로 찾는다.
> 이 표 밖의 라운드 9 일(계약과 무관): frontend-dev의 `posts/index.json` 시각 정정(meeting-08 U5 — 단독 커밋) · 골든 벡터(U4 재개분)는 회의록 지시대로.

| # | 담당 | 파일 · 위치 | 할 일 | 근거 | 상태 |
|---|---|---|---|---|---|
| 160 | web-designer | `prose.css` §4 | `.prose p:has(+ ul, + ol)` 아래 `--sp-3` · `.prose p + ul/ol` 위 `--sp-3` · `.prose li > p` 0 · `.prose li > p + p` 위 `--sp-3` | §5-7 | ✔ |
| 161 | web-designer | `docs/contract.md` | v4.3 개정(이 절 포함) | — | ✔ |
| 162 | frontend-dev | `markdown.js` 새 `codeRanges(text)` · `codeAt(text, pos)` · `Blog.markdown` export | CommonMark 펜스 스캐너(``` · `~~~` · 0~3칸 · 닫는 길이 ≥ 여는 길이 · 백틱 정보 문자열에 백틱 없음 · 목록·인용 컨테이너). 모양 `{open, bodyStart, bodyEnd, close, lang}` 그대로. marked 없이, 메모 1칸. **합격 표 11행**을 스크래치 헤드리스로 marked 12.0.2 `lexer`의 펜스 `code` 토큰과 대조 → 결과를 frontend-dev-2에게 | §6-2 "코드 블록 안 = `codeRanges`" | ☐ |
| 163 | frontend-dev | `markdown.js` 새 `langLabel(lang)` · `enhanceCodeBlocks()` | `''` → `text`, 소문자 키로 `LANG_LABEL`, 없으면 받은 그대로. 머리띠 라벨·복사 버튼 `aria-label`이 이 함수를 쓴다(화면 결과는 v4.2와 같다) | §6-2 같은 절 · §5-7 | ☐ |
| 164 | frontend-dev | `post.js` `render()` | `!post.frontmatterOk`면 `Blog.admin.isAdmin()`일 때만 토스트, 문구는 `store.findMeta(id)` 유무로 둘 중 하나(글자 그대로). 방문자는 토스트 없음 | §5-6 "깨진 frontmatter의 화면" | ☐ |
| 165 | frontend-dev | `config.js` `storageKeys` | `codeLang: 'blogCodeLang'` 추가(주석: §3-2 저장 키 표) · `admin: 'blogAdmin'` 삭제 | §3-2 저장 키 표 | ☐ |
| 166 | frontend-dev | `store.js` `draft` | `draft.key(id)` export — 기존 `draftKey()` 그대로(`'blogDraft:' + (id \|\| 'new')`) | §3-2 | ☐ |
| 167 | frontend-dev-2 | `write.html` `.editor-more` 파일 id `.field-group` · 낡은 주석 | `#fIdHint` 다음에 `<p class="field-error" id="fIdError" hidden></p>`(+ §6-0 마크업의 HTML 주석 한 줄). meeting-08 지시 6의 `write.html:116, 224` 낡은 주석 정리 | §6-0 마크업 | ☐ |
| 168 | frontend-dev-2 | `editor.js` 새 `lineStartAt(value, p)` · 8곳 | `updateCaret` `linePrefix` `learnFenceLang` `maybeFence` `lineAtCaret` `lineSpan` `moveLines`(`lineStartAt(value, span.start - 1)`) Shift+Tab — `lastIndexOf('\n', p - 1) + 1` 관용구 0건 | §6-2 "줄 시작" | ☐ |
| 169 | frontend-dev-2 | `editor.js` `insideFence` `codeLines` 지역 `codeAt(a, b)` `fenceLangAbove` `learnFenceLang` `maybeFence` `onBodyEnter` · `FENCE_LINE_RE` | 판정을 `Blog.markdown.codeAt`/`codeRanges`로 교체, 사본 삭제. 지역 함수 `codeAt(a, b)`는 `Blog.markdown.codeAt`과 이름이 겹친다 — 없애거나 `inCode(a, b)` 등으로 바꾼다. 주석 토글 언어 = `range.lang` | §6-2 같은 절 · "코드 블록 안의 짝" · Enter 공통 전제 | ☐ |
| 170 | frontend-dev-2 | `editor.js` `insertCodeBlock()` | 코드 안·펜스 줄 걸침이면 넣지 않고 토스트 `이미 코드 블록 안이에요. 블록 밖에서 눌러 주세요` · 선택이 `\n`으로 끝나면 그 줄바꿈을 닫는 펜스 뒤로 | §6-2 툴바 코드 블록 행 | ☐ |
| 171 | frontend-dev-2 | `editor.js` `FENCE_TRIGGER_RE` `CODE_LANG_RE` `FENCE_OPEN_LANG_RE` · `maybeFence` `rememberCodeLang` `codeLang` | 글자 집합 `[A-Za-z0-9+#-]`, 친 그대로 쓰기·저장, 비교(주석 표)만 소문자. `//Java` + Enter → ```` ```Java ```` | §6-2 `//` 행 · 언어 기억 | ☐ |
| 172 | frontend-dev-2 | `editor.js` `onBodyKeydown()` Tab 갈래(선택 있음) | 줄 편집을 `applyLineEdits()`로 — 선택은 `map`으로 옮긴다(줄 전체 선택 폐기) | §6-2 Tab 행 · "편집 뒤 선택" | ☐ |
| 173 | frontend-dev-2 | `editor.js` `COMMENT_*` `commentStyle()` `toggleComment()` | 표 확장(`#` `--` `;` `REM` `%` · 감싸기 추가분), 표에 없음·언어 없음·주석 없는 언어 = 글 그대로 + `preventDefault` + 토스트 두 문구 | §6-2 "주석 토글" | ☐ |
| 174 | frontend-dev-2 | `editor.js` 문서 `keydown`(Ctrl+S) | `e.key.toLowerCase() !== 's'` → `!keyIs(e, 's')`. 보조키 조건(Ctrl 또는 Cmd)은 v3.9 그대로 | §6-2 Ctrl+S 행 | ☐ |
| 175 | frontend-dev-2 | `editor.js` `CODE_LANG_KEY` | `(CFG.storageKeys && CFG.storageKeys.codeLang) \|\| 'blogCodeLang'` | §3-2 · 언어 기억 "키의 자리" | ☐ |
| 176 | frontend-dev-2 | `editor.js` `saveToServer()` · `showFieldError()` · `dom` | 저장 순서 ②: 새 글이거나 id를 바꿨으면 `store.loadIndex(true)` → `store.findMeta` — 자동 id면 접미사 `-2`…`-9`(+ `#fId` 값 갱신), 손 id·접미사 소진이면 `#fIdError`(문구 셋). `#fId`에 `#editorMore.open = true` 뒤 포커스(여는 것만). `#fId` `input`에서 지우기. **분류 PUT보다 먼저** | §6-1 "저장 순서와 409" · §6-0 오류 규칙 | ☐ |
| 177 | frontend-dev-2 | `editor.js` `doSave()` · `apiErrorMessage()` · 불러오기 · 새 `state.originalUpdated` | PUT에 새 글 `ifNew: true` / 디스크 글 `expectedUpdated`. 409 `exists`(자동 = 접미사로 글 PUT만 재시도 · 손 = `#fIdError`) · `stale`(확인 모달 `덮어쓰기`/`취소`, 덮어쓰기 = `expectedUpdated` 없이 한 번 더 · `currentUpdated: null`이면 `다시 만들기` 모달) · `exists`의 `path: null`은 `index.json에만 있음` · `conflict`·코드 없음(꼬리 `. 파일을 손으로 정리한 뒤 다시 시도해 주세요`는 여기만). 상태줄 문구는 §6-1 목록 | §6-1 같은 절 | ☐ |
| 178 | frontend-dev-2 | `editor.js` `onSaved()` | 자동 접미사를 썼으면 토스트 `같은 id의 글이 있어 새 id로 저장했습니다: <새 id>` · `ifNew`를 보냈는데 `isNew === false`면 확인 모달(`같은 id의 글을 덮어썼어요` · 2행은 `git.committed`로 갈림) — 이때 저장 토스트는 없다 | §6-1 같은 절 | ☐ |
| 179 | frontend-dev-2 | `editor.js` `onBodyKeydown()` 조합 가드 · `onBodyInput()` | 조합 가드가 돌려보낸 닫는 글자에 한 번짜리 표시 → 다음 `input`에서 뒤의 같은 글자 하나 지우기 · 표시 버림 규칙. **U7 실기 확인 뒤**, 229가 안 오면 "해당 없음"으로 닫는다 | §6-2 "조합 확정 뒤 건너뛰기" | ☐ |

**확인 방법(담당자 셀프체크 — 입력은 §0-4)** — #162: 합격 표 11행 전부 일치(행마다 `[bodyStart, bodyEnd)` 줄 목록을 marked 토큰과 나란히 출력), marked를 싣지 않은 페이지에서도 결과가 같다.
#164: 방문자 조건(hostname 바꾼 사본 또는 `isAdmin` 스텁)에서 `---` 앞 빈 줄 글 → 토스트 없음 / 관리자 → index에 있는 글·없는 글 문구 둘.
#168: `"\n  foo"` 커서 0 Shift+Tab → 변화 없음 · `"\nabc"` 커서 0 H2 → 예외 없음 · 커서 2 Alt+Shift+↑ → `"abc\n"` · 계기판 `1행 1열`.
#169·#170: `~~~js` 블록·목록 안 펜스·백틱 4개 안(§0-4 ③ 외부 마크다운)에서 `(` → 짝 / 코드 안 `- a` 줄 Enter → `- ` 이어쓰기 없음 / 코드 안 줄 첫머리 `//` + Enter → 펜스 안 생김 / 코드 안에서 툴바 코드 블록 → 토스트, 글 그대로 /
줄 첫머리 ```` ```x``` ```` 아래 문단에서 `(` → 짝 없음. #171: `//Java` + Enter → ```` ```Java ````, 다음 툴바 → ```` ```Java ````에 `Java` 선택(H1). #172: 코드 안 `foo bar`에서 `bar`만 골라 Tab → `bar` 선택 유지, 한 글자 쳐도 줄이 남는다.
#173: ```` ```powershell ```` 안 Ctrl+/ → `# ` / ```` ```json ```` · 언어 없는 블록 → 글 그대로 + 토스트. #174: 합성 `key:'ㄴ', code:'KeyS', ctrl` → `defaultPrevented: true`.
#176-178: fetch 스텁 하네스(meeting-08 dev-2 지시 1 — 200 / 200 + `isNew:false` / 409 `exists` / 409 `stale` / 409 `conflict` / 500 / 네트워크 실패)로 각 행의 화면·상태줄·토스트 문구 대조 +
같은 날 `Java 정리` → `Java 복습` 새 글(H6)에서 둘째가 `…-java-2`로 저장되고 토스트가 알린다 / 세부 설정에 손으로 기존 id → 세부 설정이 열리고 `#fIdError` + 포커스, 분류 PUT이 나가지 않는다.
Docker 실측(`/api-check`)은 pm이 U1 뒤에.

### 12-20. v4.4 이행 — 전구 인트로 폐기 · 페이지 올라오기 · 제목 타자 (#180-187)

> **계약 먼저, 그다음 코드.** 규칙 전문은 §3-5. 여기는 파일별 할 일과 합격 기준뿐이다. 이 표 밖의 동작·문구·저장 키·클래스를 만들지 않는다.
> **두 개발자 사이의 접점은 `html[data-intro]`의 뜻 하나**(§3-5 "조건") — frontend-dev는 쓰는 쪽(`theme-init.js`)과 타자(`app.js`), frontend-dev-2는 sessionStorage를 쓰는 쪽(`ui.js`).
> 서로의 파일을 기다릴 필요가 없다(순서 무관). 한 파일을 두 사람이 만지지 않는다(§1-1). 줄번호보다 함수 이름으로 찾는다.
> **#184 · #187이 끝나기 전에는 배포하지 않는다** — CSS가 먼저 바뀌어 옛 `.intro` 마크업이 스타일 없이 남는다(머리말 "이행 상태").

| # | 담당 | 파일 · 위치 | 할 일 | 근거 | 상태 |
|---|---|---|---|---|---|
| 180 | web-designer | `docs/contract.md` | v4.4 개정(이 절 포함) | — | ✔ |
| 181 | web-designer | `tokens.css` · `base.css` | 인트로 토큰 일곱(`--z-intro` `--dur-intro-in` `--delay-intro` `--dur-intro-on` `--hold-intro` `--dur-intro-exit` `--dur-intro-out`) · `--c-glow`(라이트·다크 값 · `@property`) 삭제 → `--dur-intro` `--dur-caret-blink` 신설. `base.css` `:root` 전환 목록에서 `--c-glow` 삭제 | §2 | ✔ |
| 182 | web-designer | `layout.css` §10 · 머리 주석 · `base.css` 모션 감소 주석 | 막·`intro-out`·파생 시각·done 규칙·reduce 블록 삭제 → `intro-arrive`(0ms · 12px · `backwards`) · `title-wait`, 둘 다 `(prefers-reduced-motion: no-preference)` 안에 | §3-5 ① · ② · ④ | ✔ |
| 183 | web-designer | `components.css` §16 | 전구 그림·키프레임 넷·`.intro-skip` 삭제 → `.title-type` `.title-typed` `.title-rest` · 커서 `::before` · `title-caret-blink` | §3-5 ② | ✔ |
| 184 | frontend-dev | `index.html` `<body>` 머리 | `<div class="intro" id="intro">…</div>`와 그 위 HTML 주석(인트로 전등 설명)을 지운다 → 스킵 링크가 `<body>`의 첫 자식. `h1.page-title[data-site-title]`의 정적 마크업·문구는 그대로. CSP·`<link>`·`<script>` 변경 없음 | §3 · §3-1 · §3-5 | ✔ |
| 185 | frontend-dev | `app.js` 새 타자 함수(이름은 담당 판단, export 없음) · `start()` · `fillSite()` | §3-5 ② 전부 — 조건 셋 · DOM(`.sr-only` + `.title-type` > `.title-typed` + `.title-rest`, `textContent`만) · 글자 단위 · 첫 글자 = `title-wait`의 끝 · 간격 120ms · 첫~끝 글자 1200ms 상한 · 깜빡임이 끝나면 평문 · 안전망 첫 글자 뒤 5000ms(§8-1). `fillSite()`는 타자 중인 h1에 대입하지 않는다 — 같은 제목이면 무동작, 다르면 타자를 끝내고 평문. `start()` 안의 호출 자리는 `Blog.ui.initShell()`과 순서 무관(누구도 `data-intro`를 바꾸지 않는다) | §3-5 ② · §8-1 | ✔ |
| 186 | frontend-dev | `theme-init.js` 셋째 IIFE 주석 · `config.js` `storageKeys.intro` 주석 | **코드 변경 없음**(`data-intro="done"` 부착 · 키 `blogIntro` 그대로). 주석의 "인트로 전등 · 막 · display:none · ui.js가 쓰기만 하고 이 속성은 붙이지 않는다"를 §3-5 "조건"의 말로 — 첫 방문 연출 · 첫 페인트 뒤에는 누구도 바꾸지 않는다 · 쓰는 곳은 `ui.js initIntro()`(index.html만) | §3-5 조건 · §3-2 저장 키 표 | ✔ |
| 187 | frontend-dev-2 | `ui.js` `initIntro()` · 머리 주석 · 절 주석 | §3-5 "조건"대로 줄인다 — `<body>`의 직계 `<main>`이 `.list-page`이고 `html[data-intro]`가 `done`이 아니면 `sessionStorage[storageKeys.intro] = '1'`(try/catch), 그 밖에는 아무것도 하지 않는다. **지운다**: `#intro`·`#introSkip` 조회 · `inert` 붙이기/떼기 · Esc 핸들러 · `animationend` · 안전망 타이머 · `finish()` · `data-intro` 부착 · 포커스 이동 · `parseCssTime()` · `INTRO_FALLBACK_MS`. `Blog.ui.initIntro` export와 `initShell()`의 호출은 그대로 | §3-5 조건 | ✔ |

**확인 방법(담당자 셀프체크 — 샌드박스 사본, `/fixture`)** — 입력: 정적 제목 `메모 블로그`(6글자, 공백 포함) · 사본에서 바꾼 제목 셋(20자 · 이모지 섞임 · NFD 한글) · 사본 `posts/index.json` `site.title`이 정적 제목과 다른 경우.
- **#184**: `document.body.firstElementChild`가 스킵 링크. `grep -n "intro" index.html` 0건(주석 포함).
- **#185** ① 새 세션 첫 로드: 약 0.6초에 첫 글자 → 한 글자씩 → 끝 글자 뒤 커서 3회 → 약 4.2초에 `h1.children.length === 0` · `h1.textContent === '메모 블로그'`.
  ② 글자가 나올 때마다 `.title-type`의 `getBoundingClientRect().left` 변화 ≤ 1px, 평문 복원 전후 제목 왼쪽 끝 변화 ≤ 1px(360 · 1440).
  ③ 타자 중 접근성 트리의 h1 이름이 `메모 블로그` 하나(`.title-type` 아래는 트리에 없다).
  ④ 같은 세션 새로고침 → 타자 DOM이 한 번도 생기지 않는다(h1에 자식 요소 생성 0). ⑤ reduced-motion 에뮬레이션 + 새 세션 → 같다(첫 페인트부터 완성).
  ⑥ 스크립트가 700ms 이상 늦는 로드(스텁) → 제목이 0.6초에 완성된 채 보이고 그 뒤 비워지지 않는다. ⑦ JS 끔 → 0.6초 뒤 정적 제목. ⑧ 숨은 탭으로 연 로드 → 타자 DOM 0.
  ⑨ 20자 제목 → 첫~끝 글자 ≤ 1200ms, 첫 글자에서 평문까지 ≤ 4.2초 · 이모지·NFD 한글이 반쪽으로 나오지 않는다. ⑩ index.json 제목이 다르면 → 도착 즉시 새 제목 평문(스팬·커서 없음), 같으면 타자가 끊기지 않는다.
- **#186**: `grep -n "전등\|막" js/theme-init.js js/config.js` 0건. 새 세션 첫 로드 동안 `html`에 `data-intro` 없음, 새로고침 뒤 `done`.
- **#187**: 새 세션 index.html → `sessionStorage.blogIntro === '1'`이고 그 로드 동안 `html[data-intro]` 변경 0(MutationObserver) · post.html부터 연 새 세션 → `blogIntro` 없음 → 그다음 index.html에서 연출 ·
  연출 중 첫 Tab = 스킵 링크, 어떤 요소에도 `inert` 없음, Esc가 아무것도 하지 않는다 · `grep -n "introSkip\|parseCssTime\|INTRO_FALLBACK\|getElementById('intro')" js/ui.js` 0건(`inert`는 모달 코드에만).

**이행 완료(2026-09-26)** — #184-187 ✔. 위 #185의 숫자(120ms · 1200ms · 약 4.2초)와 ⑨의 기준은 **v4.5가 바꿨다** — 지금 기준은 §12-21.

### 12-21. v4.5 이행 — 제목 타자 박자 (#188-189)

> 사용자 원문(2026-09-26) *"메모블로그 글자가 너무 빨리 나와 속도 조금만 줄이고 커서 깜빡이는 시간 살짝만 늘려"*. 규칙 전문은 §3-5 ②(간격 · "5초 예산") · §8-1. 동작·DOM·이벤트·조건은 v4.4 그대로 — **숫자만** 바뀐다.

| # | 담당 | 파일 · 위치 | 할 일 | 근거 | 상태 |
|---|---|---|---|---|---|
| 188 | web-designer | `docs/contract.md` · `tokens.css` `--dur-caret-blink` · `components.css` §16 주석 | v4.5 개정 · `--dur-caret-blink` 1000ms → **1100ms**(주석의 500/3초/시간표 → 550/3.3초/4.7초) · 횟수 3 그대로 | §2 · §3-5 ② | ✔ |
| 189 | frontend-dev | `app.js` 제목 타자 상수(`TYPE_STEP_MS` · `TYPE_SPAN_MS`) · 절 머리 주석 | 글자 간격 120 → **160** · 첫~끝 글자 상한 1200 → **1600**. **안전망 `TYPE_NET_MS` 5000은 그대로.** 간격 계산식·사건 기점·`animationend` 처리 변경 없음. 상수 옆 주석에 숫자가 있으면 함께 | §3-5 ② "간격" · §8-1 | ✔ |

**확인 방법(샌드박스 사본, `/fixture`)** — 입력은 §12-20과 같다(정적 제목 `메모 블로그` · 사본에서 바꾼 20자 · 이모지 · NFD 한글).
- **#188**: `getComputedStyle(document.documentElement).getPropertyValue('--dur-caret-blink').trim() === '1100ms'`. 끝 글자 뒤 `.title-rest`의 `::before` 애니메이션이 `iterations 3` · `duration 1100` · 끝 opacity 0.
- **#189** ① 새 세션 첫 로드: 첫 글자 ≈600 → 끝 글자 ≈1400(글자 사이 160 ± 한 프레임) → 평문 ≈4700(`h1.children.length === 0` · `textContent === '메모 블로그'`).
  ② 20자 제목 → 첫~끝 글자 ≤ 1600ms · 첫 글자에서 평문까지 ≤ 5000ms이고 **평문이 된 원인이 `animationend`**(안전망 아님 — 로그로 구분). ③ v4.4의 나머지 합격 기준(§12-20 #185 ②~⑧ · ⑩)은 그대로 통과.
...
사용자 원문(2026-09-26, v4.4 미리보기를 보고) *"메모블로그 글자가 너무 빨리 나와 속도 조금만 줄이고 커서 깜빡이는 시간 살짝만 늘려"*. v4.4 안의 수정이 아니라 새 판으로 올린 이유 — v4.4는 #180-187로 구현·검증이 끝난 기준선이고, 그 합격 숫자(4.2초)를 지우지 않고 남겨야 다음에 박자를 또 만질 때 무엇에서 얼마나 옮겼는지가 보인다.

| 절 | v4.4 | v4.5 |
|---|---|---|
| 머리말 | v4.4 블록이 맨 위 · 이행 #184-187 ☐ | **v4.5 블록** · v4.4 블록에 이행 완료 한 줄 |
| §1-2 원칙 4 | 6글자 4.2초 · 5초 미만 | 6글자 4.7초 · **첫 글자부터** 5초 미만 |
| §2 | `--dur-caret-blink` 1000(500 + 500) · 3초 | **1100**(550 + 550) · 3.3초 — 회수가 아니라 주기를 늘린 근거 |
| §3-5 ② | 간격 120 · 상한 1200 · 시간표 1200 / 4200 · 깜빡임 3초 | 간격 **160** · 상한 **1600** · 시간표 1400 / 4700 · 깜빡임 3.3초 · **"5초 예산" 문단 신설**(기점 = 첫 글자, 식 1600 + 3300 = 4900 < 5000) |
| §8-1 | 120 · 1200 · 1000 × 3 · 6글자 4200 / 상한 4800 | 160 · 1600 · 1100 × 3 · 6글자 4700 / 상한 5500(첫 글자부터 4900) · 안전망 5000 그대로 |
| §11 | 커서 행 1000 | 1100 |
| §12 | §12-20 #184-187 ☐ | #184-187 ✔ · **§12-21 신설(#188-189)** |

**같은 라운드에 web-designer가 수행한 CSS 변경** — `tokens.css`(`--dur-caret-blink` 1100ms · 첫 방문 연출 주석의 시간표) · `components.css` §16(깜빡임 주석의 3초 → 3.3초).

### v4.4 — 전구 인트로 폐기 · 첫 방문 연출 = 페이지 올라오기 + 제목 타자

사용자 원문(2026-09-25) *"페이지 올라오기로 ㄱㄱ"* · *"한 글자씩 나오면서 마지막에 커서 깜빡거리는 것도 추가하자"* · *"전구 자체를 빼"*. 사용자가 확정했던 축 "인트로 전구"를 사용자 본인이 폐기했다. 소리는 넣지 않는다.

| 절 | v4.3 | v4.4 |
|---|---|---|
| 머리말 | v4.3 블록이 맨 위 | **v4.4 블록**(사용자 원문 · 바뀐 절 · 접점 · 이행 상태) |
| §1-2 | 원칙 2·3에 전구 예외(켜진 면 · 글로우) · 원칙 4의 셋째 모션 = 인트로 전등 2.25초 | **전구 예외 둘 삭제** · 셋째 모션 = 첫 방문 연출(타자는 stagger가 아니다 · 깜빡임 3회) |
| §2 | 인트로 토큰 여섯 + `--z-intro` + `--c-glow` · 파생 `--intro-exit-at` `--intro-out-at` `--bulb-*` · `@property` 41 | **전부 폐기** → `--dur-intro`(600) · `--dur-caret-blink`(1000) · `@property` 40 |
| §3 · §3-1 | index.html `<body>` 첫 자식 = `.intro`, 스킵 링크 둘째 | 세 페이지 모두 스킵 링크가 첫 자식 |
| §3-2 저장 키 표 | `intro` 행 | 쓰는 곳 · 읽는 곳 명시(키 · 값 그대로) |
| §3-5 | 인트로 전등(막 · 전구 · 건너뛰기 · `inert` · Esc · 2.25초) | **폐기. 전면 개정** — 조건(`html[data-intro]` 하나, 첫 페인트 뒤 불변) · ① 올라오기(0ms · 12px · `backwards`) · ② 제목 타자(DOM · 폭 선점 · 커서 · 깜빡임 3회 · 조건 셋 · `fillSite` · JS 실패 · reduced-motion) |
| §4-2 · §4-6 ⑧ | "인트로" | 첫 방문 연출 |
| §7 · §8 | `.intro` `.intro-bulb` `.intro-skip` · `inert` · `#intro[hidden]` · `finish()`가 `data-intro` 부착 | **폐기** → `.title-type` `.title-typed` `.title-rest` · 상태 = `data-intro` 하나 + 내용(`:empty`) |
| §8-1 · §8-2 | 인트로 종료 2250ms · 골격 셋 0 시작 | 제목 타자 행(120ms · 1200 상한 · 안전망 5000) · `title-wait`가 §8-2 위반이 아닌 이유 |
| §11 · §11-1 | 인트로 여섯 행 · layout §10 / components §16 = 막 / 전구 | 세 행(`intro-arrive` · `title-wait` · `title-caret-blink`) · layout §10 = 골격 / components §16 = 타자 부품 |
| §12 | §12-19까지 | **§12-20 신설(#180-187)** — #180-183 ✔ · §12-12에 폐기 표시 |

**같은 라운드에 web-designer가 수행한 CSS 변경** — `tokens.css`(인트로 토큰 일곱 · `--c-glow` 둘 · `@property` 하나 삭제, `--dur-intro` · `--dur-caret-blink` 신설, 모션·z-index 주석) ·
`base.css`(`:root` 전환 목록 `--c-glow` 삭제 · 모션 감소 주석) · `layout.css`(§10 전면 · 머리 주석) · `components.css`(§16 전면).

### v4.3 — 덮어쓰기 차단 · 에디터 판정 = 렌더러 · 사용자 습관 입력

meeting-08 "다음 라운드 작업 지시 → web-designer" 2~7번 + 사용자 결정 U2 · U3 · U4 · U11(2026-09-23). 사용자 원문 *"그냥 싹다 고쳐"*.

| 절 | v4.2 | v4.3 |
|---|---|---|
| 머리말 | v4.2 블록이 맨 위 | **v4.3 블록**(축 셋 · 사용자 결정 넷 · 바뀐 절 · 이행 상태) |
| §0 | §0-3까지 | **§0-4 신설 — 사용자 습관 목록 H1~H7**(관찰 근거 파일:줄) + "✔ = 습관 행을 입력으로 돌렸다" 규칙 + 검증 입력 세 묶음 |
| §3-2 | 저장 키가 절마다 흩어짐 · `storageKeys.admin` 죽은 키 · `blogCodeLang`은 `editor.js` 상수 | **저장 키 표**(7개 + `admin` 삭제) · `codeLang` 등재 · `Blog.store.draft.key(id)` export |
| §5-6 | 파싱 경고 토스트가 방문자에게도 · 문구가 index에 없는 글에서 거짓 | **깨진 frontmatter의 화면** — 방문자 토스트 없음, 관리자 문구 둘, 메타 덩어리 숨김은 B8-12(규칙만 고정) |
| §5-7 | 라벨·목록 간격 = 문단 간격(20) · 느슨한 목록 항목 사이 20 · 이미지 경로 규칙 없음 | **라벨 + 목록**(12, 두 규칙 짝 — 여백 겹침 실측) · **느슨한 목록**(항목 안 문단 0) · **이미지 경로 표**(U4 "규칙만") |
| §5-7-2 | 트리거 행이 콜론 처리와 h2 요구를 한 문장에(자기모순) | h2뿐 — 문단 라벨형은 옮기지 않는다(U3 = c) |
| §6-0 | 오류 줄 넷 · `details` open 불가침 | **`#fIdError`**(다섯째) · 문구 셋 · id 확인은 `validate()` 셋 다음 · **접힌 `.editor-more`는 펼친 뒤 포커스**(유일한 예외, 여는 것만) |
| §6-1 | 409에 무조건 "손으로 정리" · `isNew` 버림 · 사전 확인 없음 | **저장 순서 ①~⑥**(id 사전 확인이 분류 PUT보다 먼저) · 자동 접미사 `-2`부터 `-9`까지 · `ifNew`/`expectedUpdated` · 응답별 화면 표(`exists` · `stale` 모달 · `conflict` 꼬리 · `isNew:false` 모달) · 상태줄 문구 셋 등재 |
| §6-2 머리 | 조합 중 "어느 것도 발동하지 않는다" | 가로채지 않되 **조합 확정 뒤 건너뛰기** 사후 보정 하나 · "펜스 안" = `codeRanges` 하나 |
| §6-2 표 | Ctrl+S `e.key`만 · 선택 있는 Tab = 줄 전체 선택 · `//` 소문자만 · 툴바 코드 블록 가드 없음 · Ctrl+/ 모르는 언어 = `//` | Ctrl+S = `keyIs` · 선택은 편집에 따라 옮김 · 대소문자 무시·친 그대로 · 코드 안이면 넣지 않고 토스트 · 줄 단위 선택의 끝 줄바꿈 · 모르는 언어 = 무동작 + 토스트 |
| §6-2 소절 | 줄 범위 · 짝 · 주석 토글 · 언어 기억 · 알리는 자리 | **신설**: 줄 시작(`lineStartAt` — 공식 → 동작 문장) · 편집 뒤 선택 · **코드 블록 안 = `codeRanges`**(접점 · 필드 · 합격 표 11행) · 조합 확정 뒤 건너뛰기. **정정**: 짝의 "펜스 안" 정의 · 주석 표(+ `;` `REM` `%` 등, 무동작 행) · 언어 기억(대소문자 · 키 = `storageKeys.codeLang`) · Enter 공통 전제 · `//` 세부("`~~~`는 세지 않는다" **폐기**) |
| `prose.css` §4 | `p`·`ul` 둘 다 `--sp-5`, `li > p` 규칙 없음 | 목록 넷(#160) |
| §12 | §12-18까지 | **§12-19 신설(#160-179)** — #160 · #161 ✔ |
| 바뀌지 않은 것 | | 클래스(신설 없음) · DOM(`#fIdError` 하나 추가 외) · `#bodyHint` 문장 · §6 `title` 규칙 · 요약 카드 동작 · 본문·제목 칸 `spellcheck`(U11 · B8-7 이월) · 푸터 문장(U2 = A) |

### v4.2 — 코드 블록 편의(사후 등재)

| 절 | v4.1 | v4.2 |
|---|---|---|
| 머리말 | v4.1 블록이 맨 위 | **v4.2 개정 블록**(사용자 원문 · 바뀐 절 · 이행 상태) + **추가 확정 문단**(§10 푸터 둘째 줄 — 처음 등재 때의 "미확정이라 등재하지 않았다"를 대체) |
| §6-0 마크업 | `#bodyHint` 자리표시 `…(v3.9 문장 그대로)…` | `…(§6-2 "알리는 자리(v4.2)"의 문장)…` |
| §6-2 머리 | "여기 없는 단축 입력을 만들지 않는다" · Cmd 언급 없음 · 잠김 언급 없음 | 같은 원칙 + "짝은 단축키가 아니다" · Ctrl = Ctrl/Cmd · 잠긴 동안 발동 없음 |
| §6-2 표 | 8행 · Ctrl 조합에 Alt/Shift 조건 없음 · 선택 없는 Shift+Tab의 커서 미명시 · 툴바 코드 블록 = ```` ```js ```` + `// 코드` | **짝 · Ctrl+/ · Ctrl+D · Alt+Shift+↑/↓ · 툴바 코드 블록(언어 기억) · 보조키 규칙** 행 신설 · Ctrl+B/I · Ctrl+S · Tab · `//` · ⑤ 행 정정 · "한 동작 = undo 한 단계" |
| §6-2 소절 | — | **신설**: 줄 범위 · 코드 블록 안의 짝(알려진 차이 포함) · 주석 토글(언어별 접두 표) · 코드 블록 언어 기억(`blogCodeLang`) · 알리는 자리(v4.2 — write.html 문구 대조표) |
| §6-2 Enter 규칙 표 | 머리 "자동 닫기 괄호는 넣지 않는다" · "하지 않는 것"의 자동 닫기 | **"펜스 밖에서는"** 으로 좁힘 · "위치와 순서"에 `onBodyKeydown()` 전체 순서 |
| §12 | §12-17까지 | **§12-18 신설(#150-159)** — #150-156 ✔(#156은 frontend-dev-2 이행 · 바이트 대조), #157 ✔(계약·CSS), #158 · #159 ✔ — 전부 ✔ |
| §3 셸 마크업 · §10 | 푸터 `.footer-note` 한 줄(요구사항 #16) + `.footer-meta` | **`.footer-note` 두 줄** — 둘째 `게시글은 모두 직접 작성하고 정리하였습니다`(사용자 추가 문구, #16과 별개) · 문장 확정 경위 표 · 간격 표 |
| `components.css` §12 | `.footer-meta` 위 `--sp-2` | `.footer-note + .footer-note` `--sp-2` 신설 · `.footer-meta` 위 `--sp-3` |
| 바뀌지 않은 것 | | 클래스(신설 없음) · DOM 구조(푸터에 `<p>` 하나 추가 외) · §6 `title` 규칙(남는 `title` 셋) · placeholder · 상태줄 문장 목록 · 요구사항 #16 문장 |
