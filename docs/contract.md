...
한글 조합 중(`isComposing` 또는 `keyCode === 229`)에는 어느 것도 발동하지 않는다. 폼이 잠긴 동안(`state.locked` · `readOnly`)에도 발동하지 않는다.
...
| Ctrl+S | 연결됨이면 저장, 아니면 안내 토스트(§6-1). **모드별 문구 없음** — 어디서나 `Ctrl+S 저장`. 문서 전체 `keydown`에서 잡는다(본문 밖에서도) — v4.2 "보조키 규칙"의 대상이 아니다(v3.9 그대로) | `#btnSave` `title`, `#bodyHint`, placeholder |
| Tab / Shift+Tab | **선택이 없으면**(`selectionStart === selectionEnd`) Tab은 커서 자리에 공백 2칸, Shift+Tab은 **현재 줄** 첫머리에서 공백 최대 2칸 제거(v3.8 그대로 — 줄 단위). **v4.2 명시: 선택 없는 Shift+Tab 뒤에도 선택은 없고, 커서는 같은 글자 앞에 남는다**(지운 칸 수만큼 왼쪽으로. 커서가 지운 공백 안에 있었으면 줄 첫머리). 첫머리에 공백이 없으면 아무것도 바꾸지 않는다(탭 문자는 지우지 않는다). **선택이 있으면 — 한 줄이든 여러 줄이든 — 선택이 걸친 줄마다** 줄 첫머리에 2칸 넣기 / 빼기, 선택은 그 줄들 전체로 유지. "걸친 줄"은 아래 "줄 범위"(v4.2 — 다음 줄 0열에서 끝난 선택은 그 줄을 뺀다). 바뀌는 글자가 없으면(빼기인데 모든 줄에 공백이 없다) 글도 선택도 건드리지 않는다. **Esc 뒤 Tab** = 포커스 이동. **v4.2: Ctrl·Alt·Cmd가 붙은 Tab은 가로채지 않는다**(탭 전환 등 브라우저·OS 몫 — Shift만 붙은 것이 Shift+Tab이다) | placeholder, `#bodyHint`, 첫 Tab 때 상태줄 |
| **`//` 줄 첫머리 (v3.8, U-1)** | 그 줄이 ```` ``` ```` 펜스 블록이 되고 커서는 블록 안 빈 줄. `//java`처럼 언어를 붙이고 **Enter 또는 Space**면 ```` ```java ````. **이미 펜스 블록 안이면 발동하지 않는다**(커서 위쪽의 펜스 줄 수가 홀수면 안) — 코드 안의 `//` 주석을 지킨다. **v4.2: 언어를 붙였으면 그 언어를 기억한다**(아래 "코드 블록 언어 기억") | 툴바 "코드블록" `aria-label`에 "(// 로도)" 병기, placeholder에 `// 로 코드 블록` 한 줄, `#bodyHint` |
| **툴바 `코드 블록` (v4.2 정정)** | **기억한 언어**(없으면 `java`)로 펜스를 연다: ```` ```<언어> ```` + 선택한 글(없으면 빈 줄) + ```` ``` ````. 넣은 뒤 **언어 글자가 선택돼 있다** — 그대로 두면 그 언어, 치면 덮어쓴다. 커서가 줄 중간이면 앞에 줄바꿈 하나를 둔다(펜스는 줄 첫머리여야 펜스다 — 빈 줄까지는 두지 않는다). v4.1까지의 ```` ```js ```` + 자리표시 `// 코드`는 폐기(언어를 바꾸면 틀린 주석이 되고 쓰기 전에 지워야 했다) | 결과가 말한다 — 선택된 언어 글자가 곧 안내다. 버튼 문구·`aria-label`은 그대로(아래 "알리는 자리") |
...
| **Ctrl+/ — 펜스 안 (v4.2)** | 걸친 줄을 주석으로 만들거나 푼다(토글). 접두는 **그 코드 블록을 연 펜스 줄의 언어**로 정한다. **펜스 밖에서는 아무것도 막지 않는다**(브라우저 기본). 표는 아래 "주석 토글" | `#bodyHint` |
...
```` ``` ````로 시작하는 줄이 **하나도 없다**(펜스를 여는·닫는 줄 자체에서는 짝·주석 규칙이 발동하지 않는다). 보조키: Ctrl·Alt·Cmd 없음(Shift는 보지 않는다 —
...
접두는 **커서 위쪽에서 마지막 펜스 줄**(= 지금 블록을 연 줄)의 정보 문자열 첫 낱말(소문자로 바꿔서)로 정한다.

| 방식 | 표시 | 언어 |
|---|---|---|
| 줄 접두 | `//` | `java` `js` `javascript` `ts` `typescript` `c` `cpp` `cs` `csharp` `kotlin` `kt` `go` `rust` `swift` `scss` `dart` `php` — **그리고 언어 없음 · 표에 없는 언어**(기본) |
| 줄 접두 | `#` | `python` `py` `bash` `sh` `shell` `yaml` `yml` `ruby` `rb` `r` `toml` `dockerfile` |
| 줄 접두 | `--` | `sql` |
| 감싸기 | `<!--` … `-->` | `html` `xml` `md` `markdown` |
| 감싸기 | `/*` … `*/` | `css` |
...
- 커서·선택은 편집을 따라 옮겨진다(지워진 글자 안에 있던 위치는 그 편집 바로 뒤로). 언어 판정은 `hasOwnProperty`로 한다 — `constructor` 같은 이름이 기본 객체의 것을 집지 않게.

#### 코드 블록 언어 기억 (v4.2 — localStorage `blogCodeLang`)

| 항목 | 규칙 |
|---|---|
| 값 | `[a-z0-9+#-]{1,20}`(`//언어` 규칙과 같은 글자 집합 — 소문자만). 없거나 규칙에 안 맞으면(손으로 고친 저장소 등) **기본 `java`**(§5-7-1 "IntelliJ — 자바 기준"과 같은 기본) |
| 쓰는 곳 셋 | ① `//언어` + Enter/Space로 펜스를 만들 때(언어 없는 `//`는 기억을 바꾸지 않는다) ② 커서가 펜스 **여는** 줄에 있고 그 줄 전체가 ```` ```언어 ````(뒤 공백만 허용)일 때 — 툴바가 선택해 둔 언어를 덮어쓴 경우 등. 정보 문자열이 더 붙은 줄(```` ```js title="a" ````)·닫는 줄·대문자(```` ```Java ````)는 배우지 않는다 ③ (읽기만) 툴바 `코드 블록` |
...
| 키의 자리 | `editor.js` 안 상수(`CODE_LANG_KEY`) — `config.js` `storageKeys`가 아니다. **이행 판단 등재(frontend-dev-2)**: 읽고 쓰는 파일이 `editor.js` 하나뿐이다. §4-6이 `storageKeys`에 둔 근거("문자열을 두 파일이 따로 적지 않는다")가 여기엔 없다. 두 번째 파일이 읽게 되면 그때 `storageKeys`로 옮긴다(frontend-dev 소유) |
...
- **위치와 순서** — `onBodyKeydown`에서 Tab 처리 **뒤**, `tabEscape` 해제 줄 **뒤**, `Ctrl` 검사 **앞**. Enter는 ①②③(펜스 안) 아니면 ④(펜스 밖)
  ...
  **v4.2 — `onBodyKeydown()` 전체 순서**: 조합 중·잠김이면 끝 → Esc(탈출 대기) → Tab/Shift+Tab(Ctrl·Alt·Cmd가 붙으면 손대지 않음) → 탈출 대기 해제 →
