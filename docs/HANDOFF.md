...
> **부분 갱신 2026-09-25 (pm)**: §9 결정 표에 v4.4 두 행(전등 소리 · 전구 인트로 폐기 → 첫 방문 연출) · v3.6~3.7 행 취소선 · §1 #7 · §2 계약 버전 · §3 `components.css` 비고만. §3 수치 · §8 · §11은 2026-09-23 기준 그대로다 — 전면 갱신은 `/handoff`.
...
| 7 | 애니메이션·슬라이드 등 (jQuery 허용) | 충족 — 콘텐츠 도착·테마 크로스페이드. 첫 방문 연출은 v4.4에서 **페이지 올라오기 + 제목 타자**로 교체(2026-09-25, 계약 §3-5 — CSS ✔ · JS #184-187 이행 중). 전구 인트로(v3.6~v4.3)는 v4.4에서 폐기 |
...
6. **사용자에게 보이는 것·입력의 결과가 바뀌면 `docs/contract.md` 먼저** — 클래스·구조 · 문구 · 키 동작 · 저장 키(2026-09-23 확대). **메인 세션도 같은 게이트.** 계약서 현재 **v4.3(커밋) / v4.5(작업 트리, 2026-09-26)**.
...
| 컴포넌트 | `css/components.css` | 2,135 | 카드 그리드·사이드바·제목 타자 부품(§16 — v4.4에 전구 인트로 자리를 대체)·에디터 |
...
| v3.6~3.7 | 사용자 요청 | 인트로 | ~~전구 인트로(첫 접속)~~ → **v4.4에서 폐기**(아래 2026-09-25 행) | 계약 §12-12(이력) |
