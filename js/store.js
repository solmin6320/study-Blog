...
(function (window, document) {
  ...
  /* ---------- 에디터 초안 (localStorage) ----------
     서버에 저장하기 전까지의 임시본만 담는다. 글의 영구 저장소는 posts/*.md 파일이다. */
  ...
  var draft = {
    ...
    /* 저장하지 않은 초안이 남아 있으면 목록 화면에서 알려 주기 위한 목록 */
    list: function () {
      var ls = safeLocal();
      if (!ls) return [];
      var out = [];
  ...
  /* ---------- 파일 직렬화 ----------
     v3.9: 브라우저 내보내기는 폐지됐다. 서버(server/posts.py)가 같은 규칙으로 .md·index.json을 쓰므로(docs/api.md §1)
     여기 규칙이 곧 파일 형식의 정의다 — 에디터가 서버 저장 경로에서 직접 부르지 않더라도 규칙의 원본으로 남긴다. */
